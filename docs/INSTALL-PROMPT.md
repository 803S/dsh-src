...
【第二步·登记与安装】
1. 在 ~/.dsh/capabilities.yaml 追加条目（文件不存在则先 cp 配置区 dsh-src 包目录内的 capabilities.yaml.example 过去）：
...
3. 若配置区「代理地址」非空且 clone 直连失败：在清单顶部加 settings: { proxy: 代理地址 } 后重跑。
4. 执行 node <配置区 dsh-src 包目录>/scripts/caps-sync.mjs。

【第三步·验证】
1. mcp 型：确认 ~/.dsh/profiles/web/cordis.patch.yml 能力标记区段里出现了 id: mcp-<id> 且 args/env 正确；提示我重启 dsh web；重启后列出工具面确认出现 mcp__<id>__ 开头的工具；实际调用其中一个只读工具做冒烟。

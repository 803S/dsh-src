...
import { keylessSearchProvider } from "./src/web-search-provider.js";
...
function apply(ctx) {
	...
	if (typeof ctx.on === "function") ctx.on("tools/pre-execute", async (exec, next) => {
		...
		try {
			...
			const decision = await layaDecide({ taskType: "tool-advisory", url: "http://tool.local/" + encodeURIComponent(name), method: "TOOL", body: raw.slice(0, 5000), headers: {} }, exec);
			const sessionId = exec.agent?.session?.id ?? "";
			const telemetry = createDefaultTelemetry();
			telemetry.emit("laya.tool-advisory", { sessionId }, { tool: name, action: decision.action, confidence: decision.confidence, source: decision.source, fallback: decision.fallback, latency: decision.latency, networkish: true });
			return next();
		} catch { return next(); }
	});

...
【finding】子 agent 仅在证据已确认时随 src_submit 提交漏洞：title、severity
  ...
  （reproducibleSteps：按顺序的复现命令/请求/动作，至少一条）和 impact 危害论证。
  impact 是 finding 的灵魂：必须写出攻击者视角的具体利用场景——怎么构造利用（恶意页面/请求/
  参数怎么放）、能实际拿到什么（数据/权限/资金/用户）、危害哪些用户或业务，缺这一层就只是
  信息罗列不算漏洞。例：CORS 反射 Origin+Credentials 必须写明「攻击者在 xxx 托管页面诱导
  已登录用户访问，JS 可读取其 yyy 接口返回的 zzz 数据」；只写「配置不安全」不合格。
...
【认证策略】发现大部分业务需登录时按决策树推进：①先用 Google dorks 被动找泄露凭证（GitHub filename:.env "<domain>"、"<domain>" password、Gitee/文库/网盘，SRC 认可白帽凭此登录测试；已知凭据复现不受爆破配额约束）；②找不到则用 src_user_todo(kind=auth-session) 异步引导用户完成 Burp 抓包三步（见【用户待办】），绝不打断任务；用户完成后 mcp__burp__get_proxy_history 拉包，src_import_traffic 完整保留认证头入库（auth-profile fact），子 agent 复用凭据构造请求测登录态越权面、判断凭据权限范围（如普通用户 token 能否读到他人数据=横向越权实锤）。安全红线：只测凭据对应账户自身的越权面，不横向；只对授权域名跑 dork。
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		description: "抓取厂商 SRC/安全应急响应中心的漏洞评分与收录规则��面正文（经会话代理出站，只读一次 GET）。用于【厂商规则】步骤：把返回正文的要点存为 fact(category=vendor-policy)。仅指挥官可调。",
		parameters: {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			if (pendingTodos.length) warnings.push(`存在 ${pendingTodos.length} 个未完成的用户待办（${pendingTodos.map((todo) => todo.id).join(", ")}）：用户待办是单点挂起而非全局暂停——除非其他方向已全部穷尽，否则不应在待办未完成时 finalize，应继续推进其他挖掘方向`);
			const coverage = view.coverage ?? [];
...
function registerTodoCommand(ctx, store) {
	...
	ctx.commands.register({
		name: "src-burp-test",
		description: "Burp MCP 连通性测试：先 TCP 探测 Burp 端口，再做 /sse 握手验证 MCP 端点真实可用，两关都过才让 agent 做 MCP 工具层验证。",
		input: { hint: "（无需参数）" },
		handler: async (invocation) => {
			...
			try {
				...
				invocation.agent.followup(createUserMessage({
					content: [{ type: "text", text: `【Burp MCP 连通性测试】服务端探测全部通过：①TCP 127.0.0.1:${port} 可达（${elapsed}ms）②MCP SSE 端点 http://127.0.0.1:${port}${sse.path} 握手成功（HTTP 200 text/event-stream，${sse.elapsed}ms${sse.preview !== "" ? `，首包：${sse.preview}` : ""}）。请立即调用 mcp__burp__get_proxy_history（limit 取 5）做 MCP 工具层验证并如实汇报：成功则报告当前代理历史条数；失败给出错误原文与排查建议（mcp-proxy.jar 是否随 dsh web 启动、jar 与 Burp 扩展版本是否匹配）。全程中文，不要臆测结果。` }],
					source: { kind: "user" }
				}));
				return { kind: "success", text: `①端口 127.0.0.1:${port} 可达（${elapsed}ms）②MCP SSE 端点 http://127.0.0.1:${port}${sse.path} 握手通过（HTTP 200，${sse.elapsed}ms）——MCP 端点确认可用（注意：该扩展仅 HTTP），已请 agent 做工具层验证，结果会出现在对话里。` };
			} catch (error) {

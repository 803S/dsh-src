...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (_args, _exec) => {
			...
			return {
				...
				items: manifest.items.map((c) => ({ id: c.id, kind: c.kind ?? "mcp", from: c.from, enabled: c.enabled !== false, status: c.status ?? (c.enabled !== false ? "declared" : "disabled"), wired: (c.kind ?? "mcp") === "mcp" ? wired.has(c.id) : void 0, when: typeof c.when === "string" ? c.when : "", docs: c.docs, scripts: Array.isArray(c.scripts) ? c.scripts : void 0 }))
			};
		}
	}));
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			if (u.hostname !== target.hostname && !u.hostname.endsWith(`.${target.hostname}`)) throw new Error("src_http target is outside the authorized goal host");
		...
		execute: async (_args, exec) => {
			...
			const orphanIntents = (view.intents ?? []).filter((intent) => intent.status === "running").map((intent) => {
				...
				if (checkpoints.length === 0) {
					return { intentId: intent.id, title: intent.title, childSessionId: void 0, lastStage: void 0, lastCheckpointAt: void 0, idleMinutes: void 0, hint: `${intent.id}「${intent.title}」running 但无任何 checkpoint（子代理从未汇报或重启丢失）→ 若已知 childSessionId 用 src_recover_child 唤醒（从未提交过 checkpoint 也可唤醒），否则新建 intent 重新委派` };
				}

...
function applySrcEvent(state, event) {
	...
	switch (event.data.name) {
		...
		case "src_update_finding": {
			...
			const patch = {};
			for (const key of [	"title", "severity", "description", "impact", "victimImpact", "affectedScope", "remediation", "entryPoint", "discoveryPath", "rawRequest", "rawResponse", "attackPrerequisites"]) {
...
function buildReport(state) {
	...
	const findingSections = state.findings.map((finding) => {
		...
		return [
			...
			`- 利用前提（钓鱼域/账号/交互要求）: ${(finding.attackPrerequisites ?? "") === "" ? "（未填写——旧数据可补写；本地.25 起新提交必填）" : finding.attackPrerequisites}`,
			`- 实际损失证据指针: ${(finding.concreteLossEvidence ?? []).length > 0 ? finding.concreteLossEvidence.join(", ") : "（未填写——须指向含敏感响应体/外带记录的 fact/observation/research id）"}`,
			`- 域名: ${domain}`,
			`- 完整 URL: ${fullUrl}`,
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const patch = {};
			for (const key of ["title", "severity", "description", "impact", "victimImpact", "affectedScope", "remediation", "entryPoint", "discoveryPath", "rawRequest", "rawResponse", "attackPrerequisites"]) if (typeof args[key] === "string") patch[key] = args[key];
			if (Array.isArray(args.pocEvidence)) patch.pocEvidence = args.pocEvidence.filter((item) => typeof item === "string" && item !== "");
			if (Array.isArray(args.concreteLossEvidence)) patch.concreteLossEvidence = args.concreteLossEvidence.filter((item) => typeof item === "string" && item !== "");

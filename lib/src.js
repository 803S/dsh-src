...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (_args, exec) => {
			...
			const assetScope = { model: "asset-inventory", totalAssets: (view.assets ?? []).length, grantableHosts: assetScopeHosts.length, distinctDomains: assetScopeDomains.size, topDomains: [...assetScopeDomains.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5).map(([host, count]) => ({ host, count })) };
			return {
				...view,
				orphanIntents,
				assetScope,

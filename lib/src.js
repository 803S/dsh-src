...
const SRC_INFRA_LABELS = Object.freeze({
	...
	burpProxyJarPath: "Burp MCP 自定义接线脚本绝对路径（自愈桥 .mjs 或旧版 mcp-proxy.jar；留空=profile patch 默认桥 ~/.dsh/tools/burp-mcp-bridge.mjs。仅记录/参考，实际接线在 cordis.patch.yml，改后需重启）",
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			let normalizedHost;
			try {
				normalizedHost = parseGoalHost(requiredString(args.target, "target"), "target");
			} catch (e) {
				/* [capability v1.2] 纯公司/品牌名（如「小米」「京东」）解析不出域名：引导先做品牌→主域确认而非硬报错。 */
				throw new Error(`${String(e.message ?? e).slice(0, 140)} 若这是公司/品牌名：先用 web_search 查其官网主域名并向用户确认授权范围，拿到明确主域名后再重新调 src_add_goal；禁止臆测域名。`);
			}

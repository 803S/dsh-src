	...
	*/
	...
	async resolvePendingApproval(sessionId, id, action, note, http, runCapability) {
		...
		if (existing.method === "ASSET") {
			...
			return { ...resolved, ...(write.nodeId !== void 0 ? { assetId: write.nodeId } : {}), duplicate: write.duplicate === true };
		}
		if (action === "allow") {
	...
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			if (assetRow !== void 0 && assetRow.method === "ASSET") {
				...
				invocation.agent.session.append("tool/call", { name: "src_resolve_approval", arguments: JSON.stringify({ id: approvalId, action, note, responseStatus: 0 }) });

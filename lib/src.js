...
【高危动作授权闸（src_http）】所有对授权目标发出的 HTTP 请求一律走 src_http 工具，不要在 bash 里用 curl/python
  直连。src_http 会自动分类：破坏性/越权/未授权删改类写请求挂起审批，用户批准后才发出，被拒/无人审则 fail-closed
  不发出。判据方法无关（信任接口命名规则做读白名单）。如果 src_http 返回 approval=rejected/approval=cancelled/approval=unavailable，
  不要重复发起同一请求、也不要绕用 bash curl 硬发——说明明用意，等用户明示后再行动。任何删账号/改他人资源/停服类
  操作即使你认为“无害验证”也必须先说明受害者全貌（ userId/账号类型/影响面）在 justification 里，让用户凭全貌判断。
...
【报告标准化模板（vulnType + attackChain）】报告按厂商提交格式生成：漏洞/情报名称、漏洞/情报类型（vulnType）、
  漏洞/情报URL、漏洞详情四节（描述&危害 / 复现证明 / 测试源信息 / 修复方案）。提交 finding 时必填 vulnType（如「登录认证漏洞」
  「越权漏洞」「信息泄露」「短信轰炸」「文件上传漏洞」「SSRF」「XSS」「逻辑漏洞」等类别名）与 attackChain（闭合叙事），
  让报告可直接用于厂商提交、无需用户手改格式。rawRequest 须为 Burp 格式原始报文（含接口地址），作为复现证明主体。
...
function formatFindingReport(finding, state, goal) {
	...
	/* App/小程序下载方式：从影响资产 meta 提取（提示词要求记录下载方式）。 */
	const appDownload = (() => {
	...
	/* 攻击链叙事：有 attackChain 直接用，无则由结构化字段拼接，让假设链无处藏身。 */
	const attackChainText = (() => {
		const chain = (finding.attackChain ?? "").trim();
		if (chain !== "") return chain;
		return [
			`① 发现：${finding.discoveryPath === "" ? "（未填接口来源链）" : finding.discoveryPath}`,
			`② 利用前提：${(finding.attackPrerequisites ?? "") === "" ? "（未填写）" : finding.attackPrerequisites}`,
			`③ 利用过程：${finding.impact}`,
			`④ 实际损失证据：${(finding.concreteLossEvidence ?? []).length > 0 ? finding.concreteLossEvidence.join(", ") : "（未填写——须指向含敏感响应体的 fact/observation/research id）"}`,
			`⑤ 受害者影响：${(finding.victimImpact ?? "") === "" ? "（未填写——须用 src_update_finding 补写）" : finding.victimImpact}`
		].join("\n");
	})();
	/* 漏洞描述：描述 + 空行 + 攻击链（含⑤受害者影响）。 */
	const vulnDesc = [
	...
	/* 复现/证明过程：保留 === Request/Response === 标记 + 代码块。 */
	const reproBlock = (() => {
		const lines = [];
		...
		if ((finding.reproducibleSteps ?? []).length > 0) {
			...
			lines.push("可复现步骤：");
			finding.reproducibleSteps.forEach((step, index) => lines.push(`Step${index + 1}: ${step}`));
		}
		return lines.length === 0 ? "（无）" : lines.join("\n");
	})();
	/* 测试源信息：从 concreteLossEvidence 指针解析实际证据内容 + 影响资产。 */
	const testSource = (() => {
		const lines = [];
	...
	const vulnType = (finding.vulnType ?? "") === "" ? `（未分类，严重度 ${finding.severity}）` : finding.vulnType;
	return [
		`### ${finding.id} ${finding.title}`,
		`漏洞/情报名称: ${finding.title}`,
		`漏洞/情报类型: ${vulnType}（严重度: ${finding.severity}）`,
		`漏洞/情报URL: ${urls}`,
		"漏洞详情:",
		"",
		"1、漏洞描述&发现方式、漏洞利用及危害",
		"",
		`漏洞接口来源: ${finding.discoveryPath === "" ? "（未填写）" : finding.discoveryPath}`,
		`前端功能点: ${finding.entryPoint === "" ? "（未填写）" : finding.entryPoint}`,
		appDownload === "" ? null : `App下载: ${appDownload}`,
		`漏洞描述: ${vulnDesc}`,
		"",
		"2、漏洞的详细复现/证明过程",
		"",
		...
		"4、修复方案",
		"",
		finding.remediation === "" ? "（未填写）" : finding.remediation,
		"",
...
function registerSrcTools(ctx, store) {
			...
			findings: {
				...
				items: {
					...
					properties: {
						...
						attackPrerequisites: { type: "string", required: true, description: "危害论证③利用前提：钓鱼页托管域要求（厂商 SRC 是否要求自有域）、需登录哪个业务/账号类型、需要的用户动作；前提不满足时不得提交。" },
	...
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: (_a, v) => [{ type: "text", text: v.approval === "allowed" ? `src_http ${v.status} ${v.method} ${v.path}（已执行，审批通过）` : `src_http 未执行：审批 ${v.approval}——${v.reason ?? ""}` }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const engagementId = await resolveEngagementSession(store, ctx, exec);
			const goal = engagementId === void 0 ? void 0 : await store.getGoal(engagementId);
			...
			/* 挂起：经 dsh 审批服务。无审批服务时 fail-closed（不发出）。 */
			const approval = ctx.get?.("approval");
			if (approval === void 0) throw new Error(`src_http 需审批但本部署未组合审批服务（dsh-user-approval）——fail-closed 不发出。分类：${verdict.category}。理由：${verdict.reason}`);
			const reason = `[${verdict.category}] ${String(args.justification ?? "").trim()} —— ${verdict.reason}`;
			let outcome;
			try {
				outcome = await approval.request({ agent: exec.agent, toolName: "src_http", callId: exec.callId, reason, ...(exec.signal ? { signal: exec.signal } : {}) });
			} catch (error) {
				throw new Error(`src_http 审批调用失败（fail-closed 不发出）：${error?.message ?? String(error)}。分类：${verdict.category}。`);
			}
			if (outcome !== "allowed-once") {
				return { method, path: u.pathname, status: 0, approval: String(outcome), reason };
			}
			const response = await http(u, { method, redirect: "manual", headers: { "user-agent": "dsh-src/1", ...headers }, ...(body !== void 0 ? { body } : {}) });
			return { method, path: u.pathname, status: response.status, approval: "allowed", reason };
		}
	}));
	ctx.tools.register(defineTool({
		name: "src_add_goal",
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			attackPrerequisites: { type: "string", required: true, description: "危害论证③利用前提：钓鱼页托管域要求（厂商 SRC 是否要求自有域）、需登录哪个业务/账号类型、需要的用户动作；前提不满足时不得提交（如厂商要求自有域而利用需任意外域）。" },
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			attackPrerequisites: { type: "string", description: "利用前提（整体重写）：钓鱼托管域要求、需登录业务、用户动作。" },
		...
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const patch = {};
			for (const key of ["title", "severity", "description", "impact", "victimImpact", "affectedScope", "remediation", "entryPoint", "discoveryPath", "rawRequest", "rawResponse", "attackPrerequisites", "attackChain", "vulnType"]) if (typeof args[key] === "string") patch[key] = args[key];
			if (Array.isArray(args.pocEvidence)) patch.pocEvidence = args.pocEvidence.filter((item) => typeof item === "string" && item !== "");
			if (Array.isArray(args.concreteLossEvidence)) patch.concreteLossEvidence = args.concreteLossEvidence.filter((item) => typeof item === "string" && item !== "");

...
import { lessonsDataDir, listAllLessons, lessonIndexLines, readLessonFile, sessionLessons, lessonsForContext, patternLessonsForContext, lessonHintLines } from "./src/lessons.js";
import { createRegisterSrcTools } from "./src/tools/index.js";
import { routePlaybookV2, PLAYBOOK_ROUTE_KEYS } from "./src/playbooks.js";
import { SRC_INFRA_DEFAULTS, SRC_INFRA_KEYS, SRC_INFRA_LABELS, dshHomeOf, readCapsManifest, capsWiredIds, capabilityCommand, runCapabilityProcess, runChildWithTimeout } from "./src/capability-loader.js";
...
import { layaDecide } from "./src/decision/laya-client.js";
import { createHttpBodyDescriber } from "./src/http-output.js";
import { createSrcStore } from "./src/store.js";
/* [local.77 §10] 事件旁账记录器：off=零新增写入；shadow=append 事件；on=另跑双折叠。 */
import { createSrcEventRecorder, srcEventKey } from "./src/event-store.js";
import { createDefaultTelemetry } from "./src/telemetry/events.js";
import { keylessSearchProvider, configureKeylessSearchInfra, withKeylessSearchInfra } from "./src/web-search-provider.js";
import { srcEventStoreFlag, srcLayaDecisionFlag, srcLayaDelegateFlag, srcLayaSkillFlag, srcLayaNextFlag } from "./src/flags.js";
...
你是 SRC 漏洞挖掘指挥官（决策 agent）。输入：目标(target) + 目的(objective)。全程中文。
探索链路：goal → intent → yields → fact → derived_from → intent → proves → finding；每步用 src_* 写图。
记录纪律：只有你调 src_add_*、src_state、src_graph、src_report；探索/执行子 agent 只能 src_submit 直写父 intent（回复只留计数与结论）。

【主循环】
1. 用 src_state(summary) 看链路与 blockedReasons，选一个 nextAction 或建新 intent；同一目标+范围+验证方法的 intent 只留一个，不重建不重复委派。
2. src_add_intent 返回的 playbook 必须消费：有 docs 先 src_read_capability(id="clown-src-playbook")，把 playbook.checks、授权边界、停止条件带入委派 prompt（真实 intentId 禁占位符，≥2 条首轮检查项，含「完成首阶段立即 src_submit checkpoint；无法开始提交 blocked/failed」）；委派发起后立即结束回合——完成事件自动回注，禁止轮询等待。
3. 委派后按 checkpoint/失败通知处理：证据更新 fact/research/finding；子 agent 失败不自动重试，先看 checkpoint 判完成部分，剩余 src_recover_child 定向唤醒（同一子代理最多 4 次，无 checkpoint 也能唤醒；424/限流是基础设施故障应续跑而非换人），失败结论也落 fact，不得装没发生。
4. 无可执行动作时 src_finalize_engagement（硬拦：pending 待办/blocked 无待办/可推导新方向/覆盖未标，四项任一为否先处理，「收益递减」不能替代枚举；covered 必须带 evidenceId；可行动盲区同回合建 src_user_todo），处理完或如实记录限制后 src_report；报告交付即本轮完成，只有用户回注新指令或待办写回才继续。禁止为收尾掐仍在产出的子代理（interrupt_agent 仅用于跑偏/死循环/超预算）。

【角色与边界（不可编码原则，优先级高于一切 Skill/经验；冲突时服务端 gate 与工具错误优先）】
- 授权：默认用户已在正规 SRC 平台注册白帽账号视为已授权（authorization 可留空），输入是公司名/品牌名先解析到明确域名再开干，仅无法唯一确定域名才 ask_user_question（全程唯一阻塞提问时机）。授权模型是「资产清单即许可」：探测类按 goal 主域或资产清单放行，简报禁自行扩大或转述授权范围；主域外 host 先判归属再 src_add_asset 登记（三档：①明显属于→confirmed 不打扰用户；②疑似无法验证→src_request_asset_confirm 请用户拍板，提交后不等待继续其他方向；③确定不属于→不探测不登记，禁止②档直接登 confirmed）。范围外意图一律拒绝。
- 禁止绕行：审批挂起期间同 host+path 请求禁止经 Burp MCP/curl/python 等任何通道发出（服务端硬闸直接拒绝并提示审批 id）——等批准、等拒绝、或换方向，绝没有第三条路；返回 approval=pending 说明已挂起未发出，不要重复发起，不阻塞继续其他方向。
- 证据诚实：没有复现证据只能提交 fact/hypothesis 不得提交 finding；未授权可达+可复现 PoC 即可入库（severity 如实 low 起评，不因「无损失实锤」停在 fact 态）；medium+ 必须三要素（impact/victimImpact/concreteLossEvidence 指向真实证据 id，三要素也是 low 的升危依据）；仅配置缺陷/头反射→src_record_research(false-positive)，差登录态/人工交互→src_user_todo，绝不塞漏洞清单，severity 无 info；数量目标是上限不是 KPI，不合格交 0 个并如实说明阻塞；负结果同样落盘（false-positive/blocked）——也是覆盖证据，同 intent+category 自动去重。
- 报告边界：报告必须区分动态实证、静态链、blocked 和未授权验证边界；finding 必填 vulnType+attackChain+复现定位（web 必填 entryPoint；rawRequest 用 Burp 格式原始报文作复现证明主体；一键脚本放 pocScript）；打回（rejected）不删除，按备注用 src_update_finding 补强后重提，禁止原样重提（准入闸拦相似二次提交）。

【恢复与交付】
- blocked/approval/orphan 按结构化 reason 处理，不轮询、不重复发送。用户待办是「单点挂起」不是「全局暂停」：创建后立即转其他不受阻方向，禁止因等待用户停摆或 finalize；因缺输入标 blocked 的同一回合必须建对应 src_user_todo（intentId 关联），建完先枚举不依赖该输入的方向，列不出才保持挂起；待办完成写回后尽快处理再继续。基础设施（代理/Burp 端口/对照账号）每次敏感动作前重读 src_get_infra 取最新值。
- 重规划三动作：src_update_intent 调优先级（1..9）/标 deprecated（failed=试败复盘；deprecated=主动放弃不阻塞收官，依据落 fact 或 coverage limitation；已完成不能废弃）/新攻击面正常新增；不留僵尸 running。
- 侦察策略：先 src_scan_surface 预检识别 WAF/CDN/挑战页、认证边界、限速；「需先绕过」不是停止信号（换 UA/降速/分块/方法编码有界试探后才 requiresDecision）；429 先退避降速，持续才停；401 是认证边界发现不是风控，继续扫完并记录；禁止无界爆破、拒绝服务、千万级字典、隐蔽大规模扫描；被动来源（dorks/页面链接/JS 静态提取/CT 日志/DNS）优先再主动测绘。
- 已安装能力是常规武器：每次建 goal 先看【已安装能力】牌面，派发前默对路由，抓取受阻是「换方法」信号不是「结束」信号（反面清单见 lessons/channel-divergence）。
- 经验库：验证某类漏洞前索引或命中先 src_read_lesson；三个必须沉淀时机（finding 收录后/人工引导从不收变收录后/关键绕过技巧）；finalize 有 finding 零沉淀会警告。
- 厂商规则：验证类工作前 web 搜厂商公开 SRC 规则，有则 src_fetch_policy 存 fact（不收清单必须单独记录），finding 提交前对照预判并在 impact 注明定级依据，厂商规则与通用定级冲突以厂商为准，找不到按通用四档（lessons/submission-quality）。
- Burp 流量：get_proxy_http_history_regex(regex=目标host) 过滤后喂 src_import_traffic(mode=mcp)；禁止用不带 regex 的 history 断言「Burp 无流量」；看不到 burp 工具≠无流量（桥降级重连窗口）。
- AI 站点/智能客服是正式测试对象（先越权面/key 暴露再间接注入，见 lessons/ai-abuse）；纯越狱或仅套系统提示词不构成独立 finding，只作注入链证据环节；测不了的场景建 src_user_todo(kind=manual-test) 移交用户。
- 续接旧会话或重启后先 src_state 看 orphanIntents（running 但子代理已失联），按 hint 补收尾或标 failed 重委派，绝不对着 running 干等。
`;
//#endregion
...
const srcProjectionSchema = z.union([z.object({
	...
	coverage: z.array(z.object({ id: z.string(), assetId: z.string().optional(), phase: z.string(), category: z.string(), status: z.enum(["planned", "running", "completed", "blocked", "not-applicable"]), evidence: z.array(z.string()), limitation: z.string(), endpointsTotal: z.number().optional(), endpointsTested: z.number().optional(), endpointsSkipped: z.array(z.string()).optional(), updatedAt: z.number() })),
	...
	observations: z.array(z.object({ id: z.string(), intentId: z.string(), assetId: z.string().optional(), method: z.string(), path: z.string(), httpStatus: z.number(), protectionSignal: z.boolean(), wafBypassed: z.boolean(), source: z.string(), decision: z.string(), respHeaders: z.string(), respBodySnippet: z.string(), reqHeaders: z.string().optional(), reqBodySnippet: z.string().optional(), createdAt: z.number() })),
...
const PROXY_REQUIRED_HOST_SUFFIXES = [
	"google.com", "googleapis.com", "gstatic.com", "googleusercontent.com", "duckduckgo.com", "html.duckduckgo.com",
	"youtube.com", "ytimg.com",
	"github.com", "githubusercontent.com", "githubassets.com", "github.io",
	"gitlab.com",
	"huggingface.co", "hf.co",
	"openai.com", "anthropic.com",
	/* [local.83] crt.sh 已实测 20s 超时不可用（用户 OPPO 实测），摘掉；agniops/certspotter 国内直连可用（<2s），不走代理。 */
...
function applySrcEvent(state, event) {
	...
	switch (event.data.name) {
		...
		case "src_add_intent": {
			...
			if (goalId !== "") {
				...
				return withNode(state, "spawns", goalId, {
					...
					playbook: routePlaybookV2(title, detail)
				}, counters);
			}
		...
		case "src_record_coverage": {
			...
			const record = { id: str(args.id) || `coverage-${state.coverage.length + 1}`, assetId: str(args.assetId) || void 0, phase, category, status, evidence: Array.isArray(args.evidence) ? args.evidence.filter((x) => typeof x === "string") : [], limitation: str(args.limitation), endpointsTotal: Number.isFinite(args.endpointsTotal) ? args.endpointsTotal : void 0, endpointsTested: Number.isFinite(args.endpointsTested) ? args.endpointsTested : void 0, endpointsSkipped: Array.isArray(args.endpointsSkipped) ? args.endpointsSkipped.filter((x) => typeof x === "string") : void 0, updatedAt: Number(args.updatedAt) || 0 };
		...
		case "src_record_observation": {
			const intentId = str(args.intentId);
			if (intentId !== "" && findNode(state, intentId, "intent") === void 0) return state;
			if (str(args.id) !== "" && state.observations.some((row) => row.id === args.id)) return state;
			const path = str(args.path);
			if (path === "") return state;
			const observation = { id: str(args.id) || `obs-${state.observations.length + 1}`, intentId, assetId: str(args.assetId) === "" ? void 0 : str(args.assetId), method: str(args.method) || "GET", path, httpStatus: Number(args.httpStatus) || 0, protectionSignal: args.protectionSignal === true, wafBypassed: args.wafBypassed === true, source: str(args.source) || "scan", decision: str(args.decision), respHeaders: str(args.respHeaders).slice(0, 600), respBodySnippet: str(args.respBodySnippet).slice(0, 1200), reqHeaders: str(args.reqHeaders).slice(0, 4000), reqBodySnippet: str(args.reqBodySnippet).slice(0, 4000), createdAt: Number(args.createdAt) || 0 };
			if (str(args.id) === "" && state.observations.some((row) => row.intentId === intentId && row.method === observation.method && row.path === path && row.createdAt === observation.createdAt)) return state;
...
const srcObservationSchema = z.object({
	...
	reqHeaders: z.string().optional(), reqBodySnippet: z.string().optional(),
...
/**
 * [local.77 §10] append-only 事件旁账行：eventId 唯一、幂等键（aggregateId:aggregateVersion:eventType）
 * 作 record key。宽松 schema：payload 是领域记录全量（随领域演化），eventType 覆盖六类领域动作 + 前向兼容。
 */
const srcEventSchema = z.object({
	id, sessionId: id,
...
/** [local.81] Phase 7 测绘种子队列行（正式领域记录，sessionId 顶层必填）。 */
...
const srcDomainSpec = defineDomain({
	...
	tables: {
		...
		/* [local.77 §10] append-only 事件旁账：领域写入先 append 后快照（flag=off 时零新增写入）。 */
		src_events: domainTable(srcEventSchema),
		/* [local.81 Phase 7] 测绘种子队列（不接 src_events：种子生命周期非领域投影对象）。 */
		survey_seeds: domainTable(srcSurveySeedSchema)
	}
});
//#endregion
//#region src/store.ts
...
const SCHEMA_OF_KIND = {
	...
	surveySeed: srcSurveySeedSchema
};
...
const SrcStore = createSrcStore({ srcDomainSpec, sharedDomainOpens, LEGACY_KEY_MIGRATION_TABLES, recordKey, TABLE_OF_ID_KIND, snapshot, SCHEMA_OF_KIND, closeProofServersOfSession, assetGrantHosts, SEVERITIES, SRC_INFRA_DEFAULTS, SRC_INFRA_KEYS, SRC_AUTH_REQUEST_BUDGET, routePlaybookV2, credentialVaultDir, writeCredential, readCredential, credentialHeaders, redactCredential, redactText, stripCredentialHeaders, CREDENTIAL_HEADER_RE, describeHttpBody, srcEventRecorder: createSrcEventRecorder(void 0) });
const registerSrcTools = createRegisterSrcTools({
	...
	routePlaybookV2, credentialVaultDir, writeCredential, readCredential, credentialHeaders, redactCredential, redactText, stripCredentialHeaders,
...
function apply(ctx) {
	...
	if (typeof ctx.on === "function") ctx.on("tools/pre-execute", async (exec, next) => {
		const name = String(exec.name ?? "");
		if (name === "web_search") {
			try {
				const engagement = await resolveEngagementSession(store, ctx, exec);
				const fetcher = makeHttpFetch(await store.getInfra(engagement ?? exec.agent?.session?.id ?? ""));
				return withKeylessSearchInfra(fetcher, next);
			} catch {}
			return next();
		}
		const networkish = name === "bash" || name.startsWith("mcp__") || name.includes("playwright") || name.includes("browser");
		if (!networkish || srcLayaNextFlag() === "off" || exec.agent?.session?.header?.parentSession !== void 0) return next();
		try {
			const raw = typeof exec.arguments === "string" ? exec.arguments : JSON.stringify(exec.arguments ?? {});
			const decision = await layaDecide({ taskType: "tool-advisory", tool: name, url: "http://tool.local/" + encodeURIComponent(name), method: "TOOL", body: raw.slice(0, 5000), headers: {} }, exec);
			const sessionId = exec.agent?.session?.id ?? "";
			createDefaultTelemetry().emit("laya.tool-advisory", { sessionId }, { tool: name, action: decision.action, confidence: decision.confidence, source: decision.source, fallback: decision.fallback, latency: decision.latency, networkish: true });
			return next();
		} catch { return next(); }
	});
	if (typeof ctx.on === "function") ctx.on("agent/pre-step", async ({ agent, signal }, next) => {
		const decision = await next();
		if (decision.kind === "reject" || srcLayaNextFlag() === "off" || agent.session.header?.parentSession !== void 0) return decision;
		try {
			const sessionId = agent.session.id;
			const view = await store.view(await resolveEngagementSession(store, ctx, { agent }) ?? sessionId);
			if (!view.initialized) return decision;
			const open = (view.intents ?? []).filter((i) => ["planned", "running", "blocked"].includes(i.status)).slice(0, 5);
			const candidates = [
				{ id: "inspect-state", label: "读取 src_state(summary) 重新确认当前阻塞与证据索引" },
				...open.map((i) => ({ id: `intent:${i.id}`, label: `推进 intent ${i.id}：${i.title}（${i.status}）` })),
				...(view.findings ?? []).filter((f) => f.status === "active").slice(0, 3).map((f) => ({ id: `verify:${f.id}`, label: `独立复核 finding ${f.id}：${f.title}` })),
				{ id: "search-knowledge", label: "对当前框架/版本/攻击面进行外部资料搜索并回填研究证据" },
				{ id: "continue", label: "继续当前模型规划，不注入额外动作" }
			];
			const picked = await layaDecide({ taskType: "next-action", stateSummary: JSON.stringify({ counts: view.counts, intents: open, findings: (view.findings ?? []).slice(0, 5), observations: (view.observations ?? []).slice(-5), pendingApprovals: view.pendingApprovals ?? [], userTodos: view.userTodos ?? [] }), candidates }, { agent });
			createDefaultTelemetry().emit("laya.next-action", { sessionId }, { action: picked.action, confidence: picked.confidence, source: picked.source, fallback: picked.fallback, latency: picked.latency, candidateIds: candidates.map((c) => c.id) });
			if (picked.fallback || picked.action === "continue" || !candidates.some((c) => c.id === picked.action)) return decision;
			return { kind: "enter", messages: [...decision.messages, createUserMessage({ content: [{ type: "text", text: `【Laya 主决策建议】${candidates.find((c) => c.id === picked.action).label}。这是建议而非权限放行；请根据当前证据执行，若不适用说明原因后选择其他方向。` }], source: { kind: "laya-next-action" } })] };
		} catch (error) {
			try { console.warn("[src] Laya next-action failed-open:", error?.message ?? String(error)); } catch {}
			return decision;
		}
	});

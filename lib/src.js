...
【外部能力路由】任务卡在客户端/逆向/端侧场景时调 src_list_capabilities 查可用外部能力（两类形态）：
  ①mcp 型——工具面出现 mcp__<id>__* 工具就直接使用；②skill 型——先 src_read_capability 读其文档
  了解用法，再用 src_run_capability 提交白名单脚本（异步挂起待审，用户批准后经 src_resolve_approval
...
【intent】先调用 src_state 检查既有 intent；同一目标、范围和验证方法的意图只能保留一个，已有等价
  ...
  委派必须明确写“父 intentId: intent-1”。委派内容还必须包含：目标、待验证任务、相关事实摘要、
  已知资产及可引用的资产 ID。【授权范围写法】简报里禁止自行扩大或转述授权范围（如「授权含 *.其他域」）——
...
【高危动作授权闸（src_http，异步挂起）】所有对授权目标发出的 HTTP 请求一律走 src_http 工具，不要在 bash 里用 curl/python
  ...
  说明受害者全貌（userId/账号类型/影响面）在 justification 里，让用户凭全貌判断。
...
async function readCapsManifest(dshHome) {
	...
	try {
		...
		return { source: "yaml", items: declared.map((c) => ({ id: c.id, kind: "mcp", from: c.from, enabled: c.enabled !== false, when: typeof c.when === "string" ? c.when : "", status: "unknown" })) };
	} catch (e) {
...
var SrcStore = class {
	...
	*/
	...
	async addFinding(sessionId, input) {
		...
		/* [local.25] finding 准入闸（危害链三要素）：info 等级已移除；缺任一要素不构成 SRC finding，拒绝信息内嵌弱信号路由。 */
		...
		const ROUTE_HINT = "危害链三要素（攻击者能力 impact / 受害者交互 victimImpact / 实际损失证据 concreteLossEvidence）缺一不构成 SRC finding；弱信号的正确去处是 research 或待办，不是漏洞清单";
		if ((input.victimImpact ?? "").trim().length < 10) throw new Error(`src_add_finding 准入拒绝：缺受害者视角（victimImpact）——谁受害、损失什么、是否可察觉。${ROUTE_HINT}`);
		const prerequisites = (input.attackPrerequisites ?? "").trim();
		if (prerequisites.length < 10) throw new Error(`src_add_finding 准入拒绝：缺利用前提（attackPrerequisites）——钓鱼托管域要求（厂商自有域？）、需登录哪个业务、需要的用户交互；前提写不清通常说明危害链未闭合。${ROUTE_HINT}`);
		...
		if (lossRefs.length === 0) throw new Error(`src_add_finding 准入拒绝：缺实际损失证据指针（concreteLossEvidence）——至少一条指向真实 fact/observation/research 的 id（含敏感响应体或外带记录）；仅请求头/响应头变化的证据不算实际损失。${ROUTE_HINT}`);
		await this.requireRef(sessionId, "intents", input.intentId, "intent");
	...
	async resolvePendingApproval(sessionId, id, action, note, http, runCapability) {
		const table = (await this.domain()).table("pending_approvals");
		...
		if (action === "allow") {
			if (existing.method === "RUN") {
				...
				/* 原样重放存储的请求：method/url/headers/body。 */
				const headers = (() => { const m = {}; for (const line of String(existing.headers ?? "").split("\n")) { const idx = line.indexOf(":"); if (idx > 0) m[line.slice(0, idx).trim()] = line.slice(idx + 1).trim(); } return m; })();
		...
		const record = snapshot({ ...existing, status: action === "allow" ? "approved" : "rejected", note: String(note ?? "") !== "" ? String(note) : existing.note, responseStatus, ...(runOutput !== void 0 ? { runOutput } : {}), updatedAt: Date.now() });
...
/* [local.26] 高危动作分类（模块一-路线A）：判定一次出站请求是否需挂起审批。
 * 拦截轴 = 写方法 + 真实目标 +（未授权 OR 越权），方法无关，信任接口命名规则做读白名单。
 * 返回 { require: bool, reason, category, victim? }。纯函数便于测试，不触网。 exported for unit testing. */
...
/** Extract a candidate "other resource id" from headers/body/query. Conservative: any small integer (1..100) attached to a resource key, or any userId header, counts as "someone else" when the request also carries an auth token (越权) — without a token, the request is 未授权 and flagged regardless of id. */
function classifyHttpRequest({ method, path, headers, body, query }) {
	...
	/* collect candidate resource ids from query/body/header */
	...
	/* explicit userId/uid header (the closeAccount red line) */
	...
	/* [local.26] 路径拆分：按 /-_/ 隔点 + camelCase 边界拆成小写 token，避免子串误伤
	 * (preset→reset, enclose→close)。强删改/读语义/副作用语义词均按整 token 命中。 */
	const tokens = String(pathStr ?? "").split(/[^a-zA-Z0-9]+/).flatMap((seg) => seg.split(/(?=[A-Z])/)).map((t) => t.toLowerCase()).filter((t) => t !== "");
	...
	/* 1) 兜底：path 强删改语义词 → 即使 GET 也挂（closeAccount/cancel 等）。 */
	...
	/* 3) 未授权删改：无 token + 写方法 + 真实目标 + path 非强读语义 → 挂。 */
	...
	/* 4) 未授权副作用触发（短信/验证码）哪怕 path 读语义也放行——发包可漏。 */
	/* 全部不命中 → 放行。 */
...
function registerSrcTools(ctx, store) {
			...
			findings: {
				...
				items: {
					...
					properties: {
						...
						attackPrerequisites: { type: "string", required: true, description: "危害论证③利用前提：钓鱼页托管域要求（厂商 SRC 是否要求自有域）、需登录哪个业务/账号类型、需要的用户动作；若漏洞需登录必须注明登录入口 URL（厂商复现要先登录才能到达漏洞点）。前提不满足时不得提交。" },
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			from: { type: "string", required: true, description: "能力来源：npm:<pkg>[@ver]、git:<url>、https://github.com/o/r、owner/repo 或 npm 包名（自动探测 registry）。" },
		...
		execute: async (args) => {
			...
			/* ③ 清单：解析现状 → 守卫式追加（已存在则跳过写入，仅重跑 sync）。 */
			let yamlText = "";
			...
			let oldIds = [];
			if (yamlText !== "") {
				try { oldIds = capsSync.parseCapsYaml(yamlText).caps.map((c) => c.id); } catch (e) { throw new Error(`src_add_capability: 现有清单解析失败，请先手动修复 ${yamlPath}：${String(e?.message ?? e)}`); }
			}
			...
			if (oldIds.includes(id)) {
				...
				await fsPromises.writeFile(yamlPath, appended.text, "utf8");
	...
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: false, properties: { pendingApprovalId: { type: "string", required: true }, dedupe: { type: "boolean", required: true }, id: { type: "string", required: true }, script: { type: "string", required: true }, url: { type: "string", required: true }, hint: { type: "string", required: true } } }, render: (_a, v) => [{ type: "text", text: `${v.dedupe ? "已存在相同待审请求，复用" : "已挂起待审"}：${v.pendingApprovalId}（${v.url}）。${v.hint}` }] },
		execute: async (args, exec) => {
			const id = requiredString(args.id, "id");
			...
			if (existing !== void 0) return { pendingApprovalId: existing.id, dedupe: true, id, script, url, hint: "同参数请求已在待审队列，勿重复提交；用 src_resolve_approval 推进" };
			...
			if (exec.agent !== void 0) appendSessionToolEvent(exec.agent.session, "src_record_pending_approval", { id: pending.id, method: "RUN", url, path: script, headers: "", body: bodyJson, category: "capability-run", reason: "外部能力脚本执行需人工授权（命令执行面）", justification });
			return { pendingApprovalId: pending.id, dedupe: false, id, script, url, hint: "脚本尚未执行。用户在「待办」tab 待审批准后调 src_resolve_approval(该 id, allow)；批准即执行，stdout/stderr 随审批结果返回" };
		}
	}));
	ctx.tools.register(defineTool({
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			headers: { type: "object", description: "请求头（Authorization/Cookie/userId 等）。用于分类越权/未授权信号。", additionalProperties: true },
		...
		execute: async (args, exec) => {
			...
			const headers = args.headers && typeof args.headers === "object" ? Object.fromEntries(Object.entries(args.headers).filter(([k, v]) => typeof k === "string" && typeof v === "string")) : {};
		...
		const pending = await store.addPendingApproval(engagementId ?? sessionId, { method, url: String(u), path: u.pathname, headers: headersText, body: storedBody, category: verdict.category, reason: verdict.reason, justification });
		/* 同步 fold：发合成事件让投影立即出现待审节点。 */
		if (exec.agent !== void 0) appendSessionToolEvent(exec.agent.session, "src_record_pending_approval", { id: pending.id, method, url: String(u), path: u.pathname, headers: headersText, body: storedBody, category: verdict.category, reason: verdict.reason, justification });
				...
				if (ownOverrides.length === 0) {
					...
					if (keys.length > 0) {
						for (const key of keys) {
							...
							if (exec.agent !== void 0) appendSessionToolEvent(exec.agent.session, "src_set_infra", { key, value: source.overrides[key] });
						}
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			attackPrerequisites: { type: "string", required: true, description: "危害论证③利用前提：钓鱼页托管域要求（厂商 SRC 是否要求自有域）、需登录哪个业务/账号类型、需要的用户动作；若漏洞需登录必须注明登录入口 URL（厂商复现要先登录才能到达漏洞点）。前提不满足时不得提交（如厂商要求自有域而利用需任意外域）。" },
	...
	ctx.tools.register(defineTool({
		name: "src_add_asset",
		description: "Register one asset of the engagement: root-domain, subdomain, ip, service, app, endpoint, mini-program, client, firmware, ai-surface, threat-intel. 授权边界（资产清单即许可）：探测类工具（src_http/src_scan_surface/src_test_bypass/src_test_credential/src_collect_*）按「goal 主域内或资产清单内」放行——要探测的 goal 主域外 host 必须先登记为资产。source 必填，写明发现或归属判定依据（crt.sh/DNS/JS 提取/品牌与业务归属判断等），这是审计线索；重复登记会合并只升状态（candidate→confirmed）；excluded 表示判定排除、不作为授权依据。Optionally link it to a parent asset (parentId, e.g. a subdomain under its root domain) so the asset graph reflects real ownership. Record parent assets BEFORE their children and reuse the returned ids.",
		parameters: {
			type: {
				type: "string",
				required: true,
		...
		execute: async (args, exec) => {
			...
			const engagementId = await resolveEngagementSession(store, ctx, exec);
			const write = await store.addAsset(engagementId ?? sessionId, {
				type: args.type,
				value: args.value,
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const updated = await store.resolvePendingApproval(engagementId ?? sessionId, id, action, note, throttledHttp, runCapability);
	...
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			if (assetRow !== void 0 && assetRow.method === "ASSET") {
				...
				const resolved = await store.resolvePendingApproval(invocation.agent.session.id, approvalId, action, note, void 0, void 0);
				/* [local.46] 合成事件统一走 appendSessionToolEvent（带唯一 callId）。此前直接 append 无 callId，
				   两条 resolve 事件在客户端会话解析撞同一 key（tool-callundefined），第二次审批即抛
				   「received more than one start Match」→ 对话历史加载失败横幅。 */
				appendSessionToolEvent(invocation.agent.session, "src_resolve_approval", { id: approvalId, action, note, responseStatus: 0 });
				/* [local.46] 直写落库的资产同步合成 src_add_asset 事件：投影（头部计数/资产 tab）与 store
				   双账本一致。此前面板显示 0 资产但闸已放行（store 有记录）。 */
				if (resolved.asset !== void 0) appendSessionToolEvent(invocation.agent.session, "src_add_asset", { type: resolved.asset.type, value: resolved.asset.value, meta: resolved.asset.meta, source: resolved.asset.source, method: resolved.asset.method, confidence: resolved.asset.confidence, status: resolved.asset.status });
	...
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			try {
				...
				/* Synthetic tool/call keeps the projection fold in sync with the direct write. [local.46] 走 helper 带唯一 callId。 */
				appendSessionToolEvent(invocation.agent.session, "src_set_infra", { key, value });
				const cleared = value === "";
	...
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			try {
				...
				for (const key of keys) {
					await store.setInfra(sessionId, key, source.overrides[key]);
					appendSessionToolEvent(invocation.agent.session, "src_set_infra", { key, value: source.overrides[key] });
				}
...
//#endregion
export { __resetSharedDomainOpensForTests, apply, inject, name, applySrcEvent, parseTodoFeedback, srcAssetSchema, srcAssetTypeSchema, srcDomainSpec, srcEdgeKindSchema, srcEdgeSchema, srcFactKindSchema, srcFactSchema, srcFindingSchema, srcGoalSchema, srcIntentSchema, srcSeveritySchema, srcInitialState, viewSrcState, classifyHttpRequest };

...
async function readCapsManifest(dshHome) {
	...
	try {
		...
		return { source: "yaml", items: declared.map((c) => ({ id: c.id, kind: c.kind === "skill" ? "skill" : "mcp", from: c.from, enabled: c.enabled !== false, when: typeof c.when === "string" ? c.when : "", status: "unknown" })) };
	} catch (e) {

...
import { lessonsDataDir, listAllLessons, lessonIndexLines, readLessonFile, sessionLessons } from "./src/lessons.js";
...
const SrcStore = createSrcStore({ srcDomainSpec, sharedDomainOpens, LEGACY_KEY_MIGRATION_TABLES, recordKey, TABLE_OF_ID_KIND, snapshot, SCHEMA_OF_KIND, closeProofServersOfSession, assetGrantHosts, SEVERITIES, SRC_INFRA_DEFAULTS, SRC_INFRA_KEYS, SRC_AUTH_REQUEST_BUDGET });
		...
		handler: async (invocation) => {
			...
			try {
				const record = await commitSyntheticMutation(invocation.agent.session, async () => {
					const next = await store.setInfra(invocation.agent.session.id, key, value);
					return { value: next, events: [syntheticEvent("src_set_infra", { key, value })] };
				}, appendSessionToolEvent);
				const cleared = value === "";

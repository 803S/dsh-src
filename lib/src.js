...
function buildReport(state) {
	...
	const findingSections = activeFindings.map((finding) => {
		...
		return [
			`### ${finding.id} [${finding.severity}] ${finding.title}${(finding.status ?? "active") === "rejected" ? " ⚠已打回" : ""}`,
			`- 漏洞描述: ${finding.description === "" ? finding.title : finding.description}`,
			"- 攻击链（发现→利用前提→利用过程→损失→受害者）:",
			...(() => {
				const chain = (finding.attackChain ?? "").trim();
				if (chain !== "") return ["  " + chain.split(/\r?\n/).join("\n  ")];
				/* 无显式 attackChain 时由结构化字段拼接，让假设链无处藏身。 */
				return [
			...
			...(finding.status ?? "active") === "rejected" ? [`- 打回备注：${finding.rejectReason ?? ""}${finding.rejectedAt ? `（${new Date(finding.rejectedAt).toISOString()}）` : ""}`] : [],
			`- 域名: ${domain}`,
			`- 完整 URL: ${fullUrl}`,
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const patch = {};
			for (const key of ["title", "severity", "description", "impact", "victimImpact", "affectedScope", "remediation", "entryPoint", "discoveryPath", "rawRequest", "rawResponse", "attackPrerequisites", "attackChain"]) if (typeof args[key] === "string") patch[key] = args[key];
			if (Array.isArray(args.pocEvidence)) patch.pocEvidence = args.pocEvidence.filter((item) => typeof item === "string" && item !== "");
			if (Array.isArray(args.concreteLossEvidence)) patch.concreteLossEvidence = args.concreteLossEvidence.filter((item) => typeof item === "string" && item !== "");

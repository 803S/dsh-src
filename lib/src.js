...
function tcpProbe(host, port, timeoutMs = 3000) {
	return new Promise((resolveProbe, rejectProbe) => {
		...
		socket.setTimeout(timeoutMs, () => { socket.destroy(); rejectProbe(new Error(`连接��时（${timeoutMs}ms 内无响应）`)); });
...
function registerTodoCommand(ctx, store) {
	...
	ctx.commands.register({
		name: "src-burp-test",
		description: "Burp MCP 连通性测试：先 TCP 探测 Burp 端口，端口可达才让 agent 做 MCP 层验证。",
		input: { hint: "（无需参数）" },
		handler: async (invocation) => {
			...
			try {
				...
				return { kind: "error", text: `Burp MCP 端口不可达：127.0.0.1:${port} 连接失败（${error?.message ?? String(error)}）。最常见原因：① Burp Pro 未安装 "MCP Server" 扩展或装了没点 Start；② 监听端口不是 ${port}（可在扩展设置里改，或修改本页「Burp MCP 端口」后重新测试）。无需重启 dsh web。` };
			}
			invocation.agent.followup(createUserMessage({
				content: [{ type: "text", text: `【Burp MCP 连通性测试】TCP 探测显示 127.0.0.1:${port} 已监听（${elapsed}ms）。请立即调用 mcp__burp__get_proxy_history（limit 取 5）做 MCP 层验证并如实汇报：成功则报告当前代理历史条数；失败给出错误原文与排查建议（mcp-proxy.jar 是否随 dsh web 启动、jar 与 Burp 扩展版本是否匹配）。全程中文，不要臆测结果���` }],
				source: { kind: "user" }
			}));
			return { kind: "success", text: `端口 127.0.0.1:${port} 可达（${elapsed}ms），已请 agent 做 MCP 层验证，结果会出现在对话里。` };
		}
	});
}
function apply(ctx) {

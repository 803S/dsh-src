...
【外部能力路由】任务卡在客户端/逆向/端侧场景时调 src_list_capabilities 查可用外部能力（两类形态）：
  ...
  清单里启用但未接线/未安装的提示用户编辑 ~/.dsh/capabilities.yaml 后运行 node <dsh-src包>/scripts/
  caps-sync.mjs 并重启 dsh；完全没覆盖的场景不要硬猜 API——如实告知用户该类任务超出当前工具面并
  建议参考 docs/CAPABILITIES.md 接入对应能力。
...
const srcPendingApprovalSchema = z.object({
	id, sessionId: id, intentId: id.optional(),
	method: z.enum(["GET", "HEAD", "OPTIONS", "POST", "PUT", "PATCH", "DELETE", "RUN"]).default("GET"),
	url: z.string().min(1),

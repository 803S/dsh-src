...
【用户待办】需要人工完成的输入（登录态、开启 Burp、提供资产等）一律用工具名 src_user_todo 异步挂起：
  创建后不要等待，立即继续其他不受阻塞的工作；用户在面板完成后状态会写回并通知你。内容纪律：
  - 标题 ≤30 字且自包含；登录类必须含完整登录站点 URL（如「登录 account.mi.com 提供会话」）。
  - detail 只写分步骤的用户操作指引（每步一句话，总长 ≤120 字），不写进度汇报或漏洞分析。
  - auth-session 类（登录态）：引导用户走 Burp 抓包三步——①打开 Burp 并给浏览器挂上代理
    ②登录目标站点 ③回到面板点「已完成」。之后你经 mcp__burp__get_proxy_history 拉包并用
    src_import_traffic(mode=mcp) 导入认证画像。禁止要求用户提供 HAR 文件或原始报文；
    仅当确认 Burp 不可用时才降级为 HAR 方案并在 detail 中说明。
  - burp-enable 类：提示启动 Burp 并确认 MCP 扩展已 Start。
【基础设施】代理、Burp MCP 端口、越权对照账号、测试手机号列表等环境参数存于 infra 设置：
  需要时先调用 src_get_infra 读取（执行短信轰炸、验证码爆破、水平越权对照前必须先读），
  用户可在 Web 面板「基础设施」页修改；src_set_infra 仅指挥官可调。testAccount 是对照账号
  凭据（user:pass 或用户名）：优先脚本登录构造低权会话做 A/B 对照，无法自动登录时创建用户
  待办请用户提供登录态，禁止臆造凭据。testPhone 可含多个号码（逗号分隔），按需选用。
...
【漏洞质量标准（SRC）】SRC 平台接受一切有实际危害的真实漏洞——包括信息泄露/指纹类（如版本暴露可匹配已知 CVE 定向利用），关键在如实论证危害：finding 必须写清实际影响与利用路径，不夸大、不虚构，severity 如实定级；只罗列信息而无任何危害论证的发现只能算 fact。报告停止前必须确认存在 medium 及以上、有可复现 POC（raw 包含接口地址）且经独立复核 verified 的 finding。
...
function parseGoalHost(raw, what = "goal.target") {
	...
	if (asciiOnly && /^[a-z][a-z0-9+.-]*:\/\//i.test(text)) {
		try {
			...
			if (url.hostname !== "" && !url.hostname.includes("xn--")) return url.hostname.toLowerCase();
		} catch {}
	}
	if (asciiOnly) {
		try {
	...
	throw new Error(`${what}「${text.slice(0, 80)}」无法解析出目标域名：请提供主域名或 URL（如 mi.com 或 https://mi.com），不要附带说明文字。可用 src_set_goal_target 修正。`);
}
...
const SRC_INFRA_LABELS = Object.freeze({
	proxyUrl: "HTTP 代理（http://127.0.0.1:7890，留空=直连；侦察抓取会走它���",
	burpMcpPort: "Burp MCP 监听端口（默认 9876，仅记录/展示用，接线在 profile patch）",
	burpProxyJarPath: "mcp-proxy.jar 绝对路径（仅记录/展示用）",
...
async function proxiedFetch(urlString, init = {}, infra = void 0) {
	...
	if (!isProxyUrl(proxyUrl)) return fetch(urlString, init);
...
var SrcStore = class {
	...
	*/
	...
	/** [local.9] Persist one infrastructure setting override (validated upstream). */
	...
	async view(sessionId) {
		const { goal, intents, facts, findings, assets, coverage, research, checkpoints, observations, userTodos, edges, infra } = await this.sessionData(sessionId);
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const updated = await store.updateGoalTarget(sessionId, parseGoalHost(requiredString(args.target, "target"), "target"));
...
/**
* Register the /src-todo human command. The command does not write state
* itself — it relays the user's panel action into the conversation as a user
* message (followup wakes an idle agent), so the agent performs the same
* src_user_todo write it would after a chat reply and stays aware of it.
* @param ctx - registrant context carrying the host commands service.
*/
function registerTodoCommand(ctx) {
	...
	ctx.commands.register({
		name: "src-infra",
		description: "基础设施设置：把 Web 面板「基础设施」页的修改转达给 agent（等价于告知 agent 调用 src_set_infra）。",
		...
		handler: (invocation) => {
			...
			invocation.agent.followup(createUserMessage({
				content: [{ type: "text", text: value === "" ? `【基础设施设置】用户在 Web 面板把基础设施设置 ${key} 清空了：请调用 src_set_infra 工具，key="${key}"、value=""（恢复默认值），并简要确认。` : `【基础设施设置】用户在 Web 面板修改了基础设施设置：key="${key}"、value=${JSON.stringify(value)}。请立即调用 src_set_infra 工具保存该修改并简要确认。` }],
				source: { kind: "user" }
			}));
			return { kind: "success", text: `已把「${key}」的修改转达给 agent。` };
		}
	});
	ctx.commands.register({
		name: "src-burp-test",
		description: "Burp MCP 连通性测试：让 agent 立即调用 Burp MCP 工具验证接线是否正常。",
		input: { hint: "（无需参数）" },
		handler: (invocation) => {
			invocation.agent.followup(createUserMessage({
				content: [{ type: "text", text: "【Burp MCP 连通性测试】用户请求验证 Burp MCP 接线。请立即调用 mcp__burp__get_proxy_history（limit 取 5）并如实汇报：①成功——报告当前代理历史条数；②失败——给出错误原文与排查建议（Burp Pro 未装 MCP Server 扩展或未 Start、监听端口不是基础设施页配置的 burpMcpPort、mcp-proxy.jar 未随 dsh web 启动）。全程中文，不要臆测结果。" }],
				source: { kind: "user" }
			}));
			return { kind: "success", text: "已让 agent 测试 Burp MCP ���接，结果会出现在对话里。" };
		}
	});
}
function apply(ctx) {
	...
	ctx.inject(["commands"], (commandCtx) => {
		registerTodoCommand(commandCtx);
	});
...
//#endregion
export { apply, inject, name, applySrcEvent, parseTodoFeedback, srcAssetSchema, srcAssetTypeSchema, srcDomainSpec, srcEdgeKindSchema, srcEdgeSchema, srcFactKindSchema, srcFactSchema, srcFindingSchema, srcGoalSchema, srcIntentSchema, srcSeveritySchema, srcInitialState, viewSrcState };

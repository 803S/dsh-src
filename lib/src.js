...
【认证策略】发现大部分业务需登录时按决策树推进：①先用 Google dorks 被动找泄露凭证（GitHub filename:.env "<domain>"、"<domain>" password、Gitee/文库/网盘，SRC 认可白帽凭此登录测试；已知凭据复现不受爆破配额约束）；②找不到则用 ask_user_question 引导用户提供已登录 Burp 请求/浏览器 HAR（须声明授权测试账号），导入后委派子 agent 测登录态业务。安全红线：只测凭据对应账户自身的越权面，不横向；只对授权域名跑 dork。
...
function registerSrcTools(ctx, store) {
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			paths: { type: "array", required: true, description: "Explicit small path list; maximum 500 paths.", items: { type: "string" } },
		...
		execute: async (args, exec) => {
			...
			const paths = [...new Set((Array.isArray(args.paths) ? args.paths : []).filter((path) => typeof path === "string" && path.startsWith("/")).slice(0, 500))];
	...
	ctx.tools.register(defineTool({
		...
		description: "从 Burp MCP / HAR / raw HTTP 文本导入已认证流量，解决登录态盲区与 React chunk 404（proxy history 里有完整真实流量）。三种模式：mcp=传入已用 mcp__burp__get_proxy_history 拿到的 flows 数组；har=传入 HAR JSON 字符串；raw=传入 raw HTTP 请求文本（Burp 格式）。仅导入 host 在授权 goal 内的流量；认证头(Cookie/Authorization/Token)脱敏入库(保留键名+前后2位掩码)；请求/响应落 observations(时间线)、endpoint 落 asset(candidate)、认证画像落 fact(脱敏)。不发新请求，只解析已有流量。",
		parameters: {
			intentId: { type: "string", required: true, description: "现有研究 intent id。" },
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: (_a, v) => [{ type: "text", text: "导入流量：" + v.observations + " 条 observation，" + v.assets + " 个 endpoint asset，" + v.authFacts + " 条认证画像(脱敏)；跳过越界 " + v.outOfScope }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const goal = await store.getGoal(sessionId);
			...
			const maskSecret = (v) => v.length <= 6 ? "****" : v.slice(0, 2) + "***" + v.slice(-2);
			const parseFlows = [];
			...
			for (const f of parseFlows) {
				...
				for (const line of authHeaders) {
					...
					await store.addFact(sessionId, { intentId, kind: "info", target: url.host, detail: `认证画像[${k}]（脱敏）：${maskSecret(v)}${args.authProfileNote ? "；用户声明：" + args.authProfileNote : ""}`, confidence: 0.9 });
				}
			}
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			try {
				...
				const aiSignals = ["openai", "anthropic", "deepseek", "qwen", "通义", "kimi", "moonshot", "gemini", "midjourney", "stable-diffusion", "chatgpt", "大模型", "llm"];
				const tiSignals = ["fofa", "shodan", "censys", "奇安信", "qianxin", "威胁情报", "ti.aliyun", "virustotal"];
				const lower = aiBody.toLowerCase();
				if (aiBody !== "" && aiSignals.some((k) => lower.includes(k))) {
					const aiId = await recordAsset("ai-surface", base.origin, `ai:detection ${aiSignals.filter((k) => lower.includes(k)).slice(0, 3).join(",")}`, "passive", "passive", .7, "candidate");
					await recordFact("info", base.origin, "发现 AI/大模型相关站点特征，建议人工测试：prompt-injection/模型越权/API key 泄露/SSRF via tool-call", .6);
					await store.upsertResearch(sessionId, { intentId, category: "ai-abuse", hypothesis: `${base.origin} AI 面可能存在 prompt-injection/模型越权/key 泄露`, preconditions: ["authorized target", "ai-surface detected"], status: "hypothesis", stopReason: "", evidence: [`auto-skeleton ai-surface ${base.origin}`, "nextStep: manual-recommended"] });
					assets.push({ id: aiId, type: "ai-surface", value: base.origin, status: "candidate", source: "passive" });
				}
				if (aiBody !== "" && tiSignals.some((k) => lower.includes(k))) {
					await recordAsset("threat-intel", base.origin, `ti:detection ${tiSignals.filter((k) => lower.includes(k)).slice(0, 3).join(",")}`, "passive", "passive", .6, "candidate");
					await recordFact("info", base.origin, "发现威胁情报平台特征，留人工测试", .5);
				}
			} catch {}
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const checkpoint = await store.addCheckpoint(parentSessionId, { intentId, childSessionId, stage, summary, facts: acceptedFacts.length, assets: acceptedAssets.length, findings: acceptedFindings.length, batchKey });

...
import { keylessSearchProvider, configureKeylessSearchInfra } from "./src/web-search-provider.js";
	...
	if (typeof ctx.on === "function") ctx.on("tools/pre-execute", async (exec, next) => {
		...
		if (name === "web_search") {
			try {
				...
				configureKeylessSearchInfra(async () => makeHttpFetch(await store.getInfra(engagement ?? exec.agent?.session?.id ?? "")));
			} catch {}
			return next();
		}

...
const srcProjectionSchema = z.union([z.object({
	...
	observations: z.array(z.object({ id: z.string(), intentId: z.string(), method: z.string(), path: z.string(), httpStatus: z.number(), protectionSignal: z.boolean(), wafBypassed: z.boolean(), source: z.string(), decision: z.string(), createdAt: z.number() })),
...
function applySrcEvent(state, event) {
	...
	switch (event.data.name) {
		...
		case "src_record_observation": {
			...
			const observation = { id: `obs-${state.observations.length + 1}`, intentId, method: str(args.method) || "GET", path, httpStatus: Number(args.httpStatus) || 0, protectionSignal: args.protectionSignal === true, wafBypassed: args.wafBypassed === true, source: str(args.source) || "scan", decision: str(args.decision), createdAt: Date.now() };
...
//#endregion
export { apply, inject, name, applySrcEvent, srcAssetSchema, srcAssetTypeSchema, srcDomainSpec, srcEdgeKindSchema, srcEdgeSchema, srcFactKindSchema, srcFactSchema, srcFindingSchema, srcGoalSchema, srcIntentSchema, srcSeveritySchema, srcInitialState, viewSrcState };

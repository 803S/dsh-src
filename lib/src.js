...
var SrcStore = class {
	...
	*/
	...
	async resolvePendingApproval(sessionId, id, action, note, http) {
		const table = (await this.domain()).table("pending_approvals");
		...
		const record = snapshot({ ...existing, status: action === "allow" ? "approved" : "rejected", note: String(note ?? "") !== "" ? String(note) : existing.note, responseStatus, updatedAt: Date.now() });
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		description: "[local.31] 审批一条挂起的高危 HTTP 请求：allow→原样重放存储的请求并返回响应码；reject→丢弃、记理由。仅决策 agent（commander）可调。用户在 SRC 面板「待办」tab 审批区点批准/拒绝后，你（agent）收到 followup 消息应立即调本工具。幂等：已 approved/rejected 的不可重复审批。",
		parameters: {
		...
		output: { schema: { type: "object", additionalProperties: false, properties: { id: { type: "string", required: true }, status: { type: "string", required: true }, method: { type: "string", required: true }, url: { type: "string", required: true }, responseStatus: { type: "number", required: true }, note: { type: "string", required: true } } }, render: (_a, v) => [{ type: "text", text: v.status === "approved" ? `已批准并发出待审请求 ${v.id}：${v.method} ${v.url} → 响应 ${v.responseStatus}。据响应推进（成功则记录证据/产 finding；失败则转其他方向）。` : `已拒绝待审请求 ${v.id}：${v.method} ${v.url} 不发出。备注：${v.note}。转其他方向。` }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const id = requiredString(args.id, "id");
			...
			const updated = await store.resolvePendingApproval(engagementId ?? sessionId, id, action, note, throttledHttp);
			...
			return { id, status: updated.status, method: updated.method, url: updated.url, responseStatus: updated.responseStatus ?? 0, note: updated.note };
		}
	}));
	ctx.tools.register(defineTool({

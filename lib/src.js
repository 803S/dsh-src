...
【报告标准化模板（vulnType + attackChain + pocScript）】报告按美团 SRC 官方模板生成：4 个字段（漏洞名称/漏洞类型/漏洞URL/漏洞级别）
  ...
  报告内容动态填充：attackChain 作为发现→利用→危害的闭合叙事直接融入第 1 节主叙述，不单独列块；
  发现方式/入口/app 下载等「根据实际情况才有」的字段只在非空时呈现，不占位、不凑数。让报告可直接用于厂商提交、无需用户手改格式。
...
function formatFindingReport(finding, state, goal) {
	...
	/* 第 1 节：漏洞描述&发现方式、漏洞利用及危害——动态填充，空字段不显示、不占位。
	   attackChain 本身就是发现→利用→危害的闭合叙事，有则作为主叙述直接融入，不再单独列【攻击链】块。 */
	const section1 = (() => {
		const lines = [];
		...
		if (chain === "" && (finding.impact ?? "") !== "") lines.push(`利用方式：${finding.impact}`);
		if ((finding.attackPrerequisites ?? "") !== "") lines.push(`利用前提：${finding.attackPrerequisites}`);
		if ((finding.victimImpact ?? "") !== "") lines.push(`受害者影响：${finding.victimImpact}`);
		if (finding.affectedScope !== "") lines.push(`影响范围：${finding.affectedScope}`);
		/* 发现方式 / 入口 / app 版本与下载：根据实际情况动态，非空才提。 */
		if (finding.discoveryPath !== "") lines.push(`发现方式：${finding.discoveryPath}`);
		if (finding.entryPoint !== "") lines.push(`入口功能点：${finding.entryPoint}`);
		if (asset !== void 0) {
			const meta = String(asset.meta ?? "");
			/* app/小程序资产：下载方式随实际情况带出，不是固定行。 */
			if (asset.type === "app" || asset.type === "mini-program" || asset.type === "webapp") {
				const dl = meta.match(/(https?:\/\/[^\s'"]+)/);
				if (dl !== null) lines.push(`应用下载：${dl[0]}`);
			}
		}
		return lines.join("\n");
	})();

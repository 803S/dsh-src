...
【推进】用 src_state 观察链路：有新事实 → 推导新 intent → 继续；证据不足 → 扩展侦察
  或换方向。
【终止】收官前自检三问，任一为否就先处理再收官：①还有 pending 用户待办吗？②每个因缺用户输入而
  blocked 的 intent 都有对应待办吗？③还有可推导的新方向吗（未完成的假设、停在自动骨架的资产、
  未派生假设的事实）？finalize 会硬性拦截这三项，「收益递减」不能替代枚举——列得出方向就必须继续。
  确已穷尽或目标达成时：先调用 src_finalize_engagement 检查未完成 intent、保护信号、未复核 finding、
  真实危害门禁和报告字段；问题处理完或明确记录限制后，再调用 src_report 产出最终报告（含每个漏洞的
  标准报告字段与可复现步骤）。报告不是终点：报告尾部会列出「⏸ 等你的事」，收到完成回注后继续推进。
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			remainingDirections: { type: "array", required: true, description: "收官自查（必填）：调用前逐项枚举当前还能推进的方向（未完成的假设、停在自动骨架的资产、未派生假设的事实、可开的新 intent）。非空数组会被拒绝——先把方向建成 intent 推进；确已穷尽传 []。", items: { type: "string" } },

...
/* [local.63] 覆盖面清单（preset/src-hunter/coverage.yaml）：goal 创建时确定性挂出的待办声明。
 * 受限子集：checklist: 头 + '  - key: value' 条目 + 4 空格平铺字符串键；{target} 占位符由调用方替换。 */
const COVERAGE_TODO_KINDS = Object.freeze(["auth-session", "burp-enable", "asset-provide", "decision", "manual-test", "other"]);
function parseCoverageYamlSubset(text) {
	const out = [];
	let cur = null;
	let sawHeader = false;
	let sawContent = false;
	for (const raw of String(text).split(/\r?\n/)) {
		...
		if (line.trim() === "" || line.trim().startsWith("#")) continue;
		sawContent = true;
		if (/^checklist:\s*$/.test(line)) { sawHeader = true; cur = null; continue; }
		const item = line.match(/^  - (.+)$/);
		if (item) {
			const kv = item[1].match(/^([A-Za-z_][\w]*):\s*(.*)$/);
			if (!kv) throw new Error("checklist 条目首行应为 '- id: xxx'");
			cur = { [kv[1]]: kv[2].trim() };
			out.push(cur);
			continue;
		}
		if (!cur) continue;
		const nested = line.match(/^    ([A-Za-z_][\w]*):\s*(.*)$/);
		if (nested) { cur[nested[1]] = nested[2].trim(); continue; }
		throw new Error(`coverage.yaml 无法解析的行：${line.trim().slice(0, 60)}`);
	}
	if (sawContent && !sawHeader) throw new Error("coverage.yaml 应以 checklist: 开头");
	if (out.length > 10) throw new Error("coverage 条目最多 10 个");
	for (const e of out) {
		if (typeof e.id !== "string" || !/^[a-z0-9-]{2,40}$/.test(e.id)) throw new Error(`coverage 条目 id 非法：${String(e.id).slice(0, 40)}`);
		if (typeof e.title !== "string" || e.title.trim() === "" || e.title.length > 80) throw new Error(`coverage ${e.id} 的 title 需为 ≤80 字符非空字符串`);
		if (e.kind !== void 0 && !COVERAGE_TODO_KINDS.includes(e.kind)) throw new Error(`coverage ${e.id} 的 kind 非法：${e.kind}（允许：${COVERAGE_TODO_KINDS.join("/")}）`);
		if (e.detail !== void 0 && (typeof e.detail !== "string" || e.detail.length > 600)) throw new Error(`coverage ${e.id} 的 detail 需为 ≤600 字符字符串`);
	}
	return out;
}
...
/* [local.63] 覆盖面清单加载：优先运行时覆盖 ~/.dsh/coverage.yaml（坏文件直接抛错，由接线处捕获不阻塞建 goal），
 * 缺失时回退仓库内置 preset/src-hunter/coverage.yaml（随 deploy 部署到 node_modules 副本）。 */
async function readCoverageChecklist(dshHome) {
	const overridePath = nodePath.join(dshHome, "coverage.yaml");
	try {
		return parseCoverageYamlSubset(await fsPromises.readFile(overridePath, "utf8"));
	} catch (e) {
		if (e?.code !== "ENOENT") throw e;
	}
	const bundledPath = nodePath.join(nodePath.dirname(fileURLToPath(import.meta.url)), "..", "preset", "src-hunter", "coverage.yaml");
	return parseCoverageYamlSubset(await fsPromises.readFile(bundledPath, "utf8"));
}
	...
	switch (event.data.name) {
		...
		case "src_add_intent": {
			...
			/* [local.58] execute 层锚点自愈的 fold 镜像：宿主日志落的是模型原始参数，模型被工具描述引导后系统性不传 goalId，
			   ...
			   语义与 execute 层一致：两锚点全漏且当前 goal 存在 → 锚到该 goal；两锚点同传仍丢弃（语义冲突）；无 goal 仍丢弃。 */
...
const registerSrcTools = createRegisterSrcTools({
	...
	assetGrantHosts, assetGrantHostsFor, hostCoveredByAssets, parseGoalHost, isPlausiblePublicHost, str, dshHomeOf, readCapsManifest, readCoverageChecklist, parseCoverageYamlSubset, capsWiredIds,

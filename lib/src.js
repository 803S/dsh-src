...
【基础设施】代理、Burp MCP 端口、越权对照账号、短信测试手机号等环境参数存于 infra 设置：
  需要时先调用 src_get_infra 读取（执行短信轰炸、验证码爆破、水平越权对照前���须先读），
  用户可在 Web 面板��基础设施」页修改；src_set_infra 仅指挥官可调。
...
const SRC_INFRA_LABELS = Object.freeze({
	...
	testAccount: "越权对照测试账号（水平越权 A/B 对照）",
	testPhone: "短信/验证码测试手机号（短信轰炸、验证码爆破类测试用）",
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		name: "src_set_infra",
		description: `写入一项基础设施设置（仅指挥官）。key 必须是：${SRC_INFRA_KEYS.join(" / ")}。value 为字符串（≤300 字符）：proxyUrl 形如 http://127.0.0.1:7890 或留空表示直连；burpMcpPort 为端口数字；httpTimeoutMs 取值 1000..60000；testPhone 为纯数字手机号。`,
		parameters: {
		...
		execute: async (args, exec) => {
			...
			if (key === "testPhone" && value !== "" && !/^\+?\d{5,15}$/.test(value)) throw new Error("testPhone 应为纯数字手机号（可带国家码 +）");

...
async function listLessonDir(dir) {
	...
	for (const name of names.filter((n) => n.endsWith(".md")).sort()) {
		...
		try {
			...
			const head = (await fsPromises.readFile(full, "utf8")).split("\n").slice(0, 3).join(" ");
...
function registerSrcTools(ctx, store) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_a, v) => [{
				type: "text",
				text: `Recorded goal ${v.id} → ${v.target}.${Array.isArray(v.lessonIndex) && v.lessonIndex.length > 0 ? `\n已有经验库（同类漏洞先 src_read_lesson 读全文再定验证方案）：\n${v.lessonIndex.join("\n")}` : ""}`
			}]
		},
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);

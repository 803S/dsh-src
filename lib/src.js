...
function buildReport(state) {
	...
	const findingSections = state.findings.map((finding) => {
		...
		return [
			...
			`- 证明截图说明: ${finding.pocEvidence.length > 0 ? "以上每条 pocEvidence 均为可直接复现的原始请求/响应文本，截图时按可复现步骤逐步执行并截取响应即可" : "（无）"}`,
			...
			findings: {
				...
				items: {
					...
					properties: {
						...
						pocEvidence: { type: "array", required: true, description: "POC 证据，必须包含可直接复现的 raw 请求/响应文本（至少含接口地址），对应标准报告的“数据包”字段。", items: { type: "string" } },
						description: {
							type: "string",

...
export function redactCredential(value) {
	...
	return text.split(/\r?\n/).map((line) => {
		const index = line.indexOf(":");
		if (index > 0 && /^(cookie|authorization|proxy-authorization|x-[\w-]*auth|token)$/i.test(line.slice(0, index).trim())) return `${line.slice(0, index + 1)} <stored>`;
...
export function stripCredentialHeaders(headers) {
	return Object.fromEntries(Object.entries(headers ?? {}).filter(([key]) => !/^(cookie|authorization|proxy-authorization|x-[\w-]*auth|token)$/i.test(key)));
}

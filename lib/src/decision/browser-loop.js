...
export function createBrowserTelemetry(exec, { telemetry, engagementId = "" } = {}) {
	...
	return (event, payload = {}) => {
		const row = {
			...
			replacedAgentTurn: payload.replacedAgentTurn ?? false,
			...payload
		};
		try {
...
export async function runBrowserLoop({ goal, executeTool, allowedUrl = false, telemetry, engagementId = "", decide = chooseBrowserCandidate }, exec) {
	...
	try {
		...
		emit("browser.observation", { observationFingerprint: "", candidateCount: 0, operation: "snapshot", targetRef: null, latency: Date.now() - observationStarted, source: "playwright", fallback: false, adopted: false, replacedAgentTurn: false, executed: true, success: false, failureCode: "snapshot_error", guardCode: "" });
		throw error;
	}
	...
	emit("browser.observation", { observationFingerprint: fingerprint, candidateCount: candidates.length, operation: "snapshot", targetRef: null, latency: Date.now() - observationStarted, source: "playwright", fallback: false, adopted: false, replacedAgentTurn: false, executed: true, success: true, failureCode: "", guardCode: "" });
	...
	emit("browser.decision", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, confidence: decision.confidence, probabilities: decision.probabilities, operation: candidate?.operation ?? "", targetRef: candidate?.targetRef ?? null, latency: decision.latency, source: decision.source, fallback: decision.fallback, adopted: decision.fallback !== true, replacedAgentTurn: decision.fallback !== true, executed: false, success: false, failureCode: "", guardCode: "" });
	let currentSnapshot;
	try {
		currentSnapshot = await runTool(MCP_TOOL_NAMES.snapshot, {});
	} catch (error) {
		emit("browser.guard", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, operation: candidate?.operation ?? "", targetRef: candidate?.targetRef ?? null, latency: 0, source: "playwright", fallback: decision.fallback, adopted: false, replacedAgentTurn: false, guardCode: "snapshot_error", executed: false, success: false, failureCode: "snapshot_error" });
		throw error;
	}
	...
	if (!guard.allowed) {
		emit("browser.guard", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, operation: candidate?.operation ?? "", targetRef: candidate?.targetRef ?? null, latency: 0, source: "hard-guard", fallback: decision.fallback, adopted: false, replacedAgentTurn: false, guardCode: guard.code, executed: false, success: false, failureCode: guard.code });
	...
	emit("browser.guard", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, operation: candidate.operation, targetRef: candidate.targetRef, latency: 0, source: "hard-guard", fallback: decision.fallback, adopted: false, replacedAgentTurn: false, guardCode: "", executed: false, success: true, failureCode: "" });
	...
	try {
		...
		emit("browser.action", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, operation: candidate.operation, targetRef: candidate.targetRef, latency: Date.now() - actionStarted, source: toolName, fallback: decision.fallback, adopted: !decision.fallback, replacedAgentTurn: !decision.fallback, executed: true, success: false, failureCode: "after_snapshot_error", guardCode: "" });
		throw error;
	}
	const after = extractSnapshotText(afterSnapshot);
	const success = afterSnapshot?.isError !== true;
	emit("browser.action", { observationFingerprint: fingerprint, candidateCount: candidates.length, chosenIndex: decision.index, operation: candidate.operation, targetRef: candidate.targetRef, latency: Date.now() - actionStarted, source: toolName, fallback: decision.fallback, adopted: !decision.fallback, replacedAgentTurn: !decision.fallback, executed: true, success, failureCode: success ? "" : "after_snapshot_error", guardCode: "" });

...
export async function layaDecide(args, exec) {
  ...
  try {
    ...
    const answers = raw.answers ?? raw; const riskScore = parseFloat(answers.risk?.score ?? "0");
    const action = answers.action?.choice ?? "pending";
    const confidence = Math.min(
      parseFloat(answers.risk?.confidence ?? "0"),
      parseFloat(answers.action?.confidence ?? "0")
    );
...
async function layaRequest(prompt, schema, fallback) {
  try {
    ...
    return { ...answers, confidence: parseFloat(answers.conf?.confidence ?? answers.choice?.confidence ?? answers.risk?.confidence ?? "0") };
  } catch (e) {
    console.warn("[laya-client] Laya 决策不可达，fail-open:", e.message);
    return fallback;
  }

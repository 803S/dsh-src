...
export function createSrcEventRecorder(domainPromise, { flag = srcEventStoreFlag, clock = { now: () => Date.now() }, randomId = randomUUID } = {}) {
	...
	return {
		...
		async appendEvent(domainOrPromise, { aggregateId, aggregateVersion, eventType, payload, sessionId }) {
			...
			/* sessionId：payload 内（领域记录自带）。[local.82 修] 顶层必须带 id（=幂等键）与 sessionId，
			   与 srcEventSchema 对齐——storage-domain 在 open 时逐行校验，缺顶层 id/sessionId 的行会让该会话所有读取全炸。 */
			await table.put(key, { id: key, sessionId: payload?.sessionId ?? sessionId, eventId: randomId(), aggregateId, aggregateVersion, eventType, payload, createdAt: clock.now() });

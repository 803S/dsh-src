...
const VALID = Object.freeze({
	...
	/* [local.90] Laya 自动审批闸 */
	DSH_SRC_LAYA_DECISION: ["off", "shadow", "on"],
	/* [local.91] Laya 主/子代理分工 */
	DSH_SRC_LAYA_DELEGATE: ["off", "shadow", "on"],
	/* [local.91] Laya Skill 排序 */
	DSH_SRC_LAYA_SKILL: ["off", "shadow", "on"]
});
...
const DEFAULTS = Object.freeze({
	...
	DSH_SRC_SURVEY: "off",
	// [local.90] Laya 自动审批闸：off=不使用；shadow=只记录不干预；on=根据置信度自动放行/拦截。
	DSH_SRC_LAYA_DECISION: "off",
	// [local.91] Laya 主/子代理分工：off=不使用；shadow=只记录不干预；on=置信度≥0.7 时自动 delegate/self。
	DSH_SRC_LAYA_DELEGATE: "off",
	// [local.91] Laya Skill 排序：off=不使用；shadow=只记录不干预；on=置信度≥0.7 时自动激活 skill。
	DSH_SRC_LAYA_SKILL: "off"
});

/** [local.93] 在 import 时缓存 env 值；随后 delete process.env[name] 仍能正确返回启动时的值。 */
const _flagCache = {};
for (const name of Object.keys(DEFAULTS)) {
	const raw = String(process.env[name] ?? "").trim().toLowerCase();
	_flagCache[name] = (raw === "" || !VALID[name].includes(raw)) ? DEFAULTS[name] : raw;
}

function readFlag(name) {
	// 优先读实时 env（保持惰性求值）；dsh 删除 env 后回退到 import 时缓存。
	const raw = String(process.env[name] ?? "").trim().toLowerCase();
	if (raw !== "" && VALID[name].includes(raw)) return raw;
	const cached = _flagCache[name];
	return cached !== void 0 ? cached : DEFAULTS[name];
}
...
/** [local.90] Laya 自动审批闸 flag */
export const srcLayaDecisionFlag = () => readFlag("DSH_SRC_LAYA_DECISION");
/** [local.91] Laya 主/子代理分工 flag */
export const srcLayaDelegateFlag = () => readFlag("DSH_SRC_LAYA_DELEGATE");
/** [local.91] Laya Skill 排序 flag */
export const srcLayaSkillFlag = () => readFlag("DSH_SRC_LAYA_SKILL");

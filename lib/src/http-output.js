...
export function createHttpBodyDescriber() {
	...
	return function describeHttpBody({ sessionId = "", url, method = "", status = 0, contentType = "", rawBody = "", secrets = [], full = false }) {
		...
		const dedupKey = `${sessionId} ${String(method).toUpperCase()} ${host} #${status} ${hash12}`;

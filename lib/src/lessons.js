...
export async function lessonsForContext(ctx = {}) {
	...
	for (const row of await listAllLessons()) {
		try {
			const text = await fsPromises.readFile(nodePath.join(row.dir, `${row.file}.md`), "utf8");
			const t = parseLessonMeta(text).triggers;
			...
			if (matchedOn.length > 0) hits.push({ file: row.file, title: row.title, source: row.source, reason: matchedOn.join("+") });
...
export function lessonHintLines(hits) {
	...
	return hits.map((h) => `【经验库命中】${h.title}（src_read_lesson id=${JSON.stringify(h.file)}${h.source === "distilled" ? "，沉淀" : ""}；${h.reason}）——先读全文再定方案，别凭记忆重走弯路`);
}

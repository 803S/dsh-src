...
/* Phase 8 pattern lessons: keep the row shape deliberately small and machine-checkable. */
export function lessonMetaOf(text) { return parseLessonMeta(text); }
export const PATTERN_COLUMNS = Object.freeze(["recognize", "try", "success", "falsePoint"]);
const PATTERN_FEATURE_ALIASES = Object.freeze({
	api: ["api", "接口", "endpoint", "rest"],
	graphql: ["graphql", "gql"],
	upload: ["upload", "上传", "文件", "附件"],
	auth: ["auth", "认证", "登录", "会话", "权限", "越权"],
	credential: ["credential", "凭证", "密钥", "token", "secret"],
	browser: ["browser", "浏览器", "前端", "dom", "xhr"],
	mobile: ["mobile", "android", "安卓", "app", "小程序"],
	cors: ["cors", "跨域", "origin"],
	business: ["business", "业务逻辑", "订单", "金额", "流程"]
});
export function patternFeatureKeys(value) {
	const hay = String(value ?? "").toLowerCase();
	return Object.entries(PATTERN_FEATURE_ALIASES).filter(([, aliases]) => aliases.some((alias) => hay.includes(alias.toLowerCase()))).map(([key]) => key);
}
export function patternFeaturesOf(text) {
	const meta = parseLessonMeta(text);
	if (meta.kind !== "pattern") return [];
	if (Array.isArray(meta.featureKeys)) return meta.featureKeys.filter((key) => typeof key === "string");
	return patternFeatureKeys([meta.vulnType, text].join(" "));
}
export function patternOf(text) {
	const meta = parseLessonMeta(text);
	if (meta.kind !== "pattern") return null;
	const sections = {};
	for (const [key, heading] of [["recognize", "认什么"], ["try", "打哪"], ["success", "出什么算成"], ["falsePoint", "假点"]]) {
		const start = new RegExp(`^##\\s+${heading}\\s*$`, "m").exec(text);
		if (start === null) { sections[key] = ""; continue; }
		const from = start.index + start[0].length;
		const rest = text.slice(from);
		const next = /(?:^##\s+|^<!--\s*lesson-meta:)/m.exec(rest);
		sections[key] = rest.slice(0, next?.index ?? rest.length).trim();
	}
	return { ...sections, id: meta.id ?? "", triggers: meta.triggers ?? {} };
}

/* A pattern must describe a shape, not smuggle a target-specific exploit into the index. */
export function patternShapeError(recognize) {
	const value = String(recognize ?? "").trim();
	if (value === "") return "pattern 的「认什么」不能为空";
	if (/https?:\/\/|\b(?:[a-z0-9-]+\.)+[a-z]{2,}\b/i.test(value)) return "pattern 的「认什么」只能写形态，不能包含域名/URL";
	if (/(^|[\s/])(?:api|admin|login|upload|reset|v[0-9]+)(?:[\s/]|$)|\b[a-z_][a-z0-9_]*(?:Id|Token|Key|Secret|Password)\b/i.test(value)) return "pattern 的「认什么」不能枚举具体 path 或字段名";
	return "";
}
...
export async function patternLessonsForContext(ctx = {}, limit = 5) {
	const requested = new Set([...(Array.isArray(ctx.features) ? ctx.features : []), ...patternFeatureKeys([ctx.target ?? "", ctx.text ?? "", ...(ctx.assetTypes ?? [])].join(" "))]);
	...
	for (const row of await listAllLessons()) {
		try {
			const text = await fsPromises.readFile(nodePath.join(row.dir, `${row.file}.md`), "utf8");
			const meta = parseLessonMeta(text);
			if (meta.kind !== "pattern") continue;
			const features = patternFeaturesOf(text);
			const matched = features.filter((key) => requested.has(key));
			if (matched.length === 0) continue;
			hits.push({ file: row.file, title: row.title, source: row.source, kind: "pattern", features: matched, reason: `pattern:${matched.join(",")}` });

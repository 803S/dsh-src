...
export function createSrcStore(dependencies) {
	...
	*/
	...
	async resolvePendingApproval(sessionId, id, action, note, http, runCapability, resolveCredential, replayOptions = {}) {
		...
		/* [local.85] 审批批准后自动建推进待办：根因=approval-1 批准后面板/父会话不知道要推进，
		   smsmo 重放结果烂在库里没人续。allow 时给父会话挂一条 userTodo 指向审批结果。 */
		if (action === "allow" && existing.method !== "ASSET") {
			try {
				await this.upsertUserTodo(sessionId, { kind: "other", title: `推进 ${id} 审批结果：${String(existing.url ?? "").slice(0, 60)} 重放状态 ${responseStatus}${responseBody ? "，响应已入库，续接验证/评估" : ""}`.slice(0, 140), detail: `面板已批准 ${id}；重放/执行结果已在 pending_approvals 记录中，父会话应续接验证与 finding 评估。` });
			} catch { /* 推进待办失败不阻塞审批落库 */ }
		}
		return record;
	}
	...
	async collectPriorContext(target, excludeSessionId) {
		...
		for (const sid of priorSessions) {
			for (const [, row] of domain.table("research").entries()) {
				if (row.sessionId === sid && (row.status === "false-positive" || row.status === "blocked")) (row.status === "blocked" ? blocked : falsified).push({ hypothesis: row.hypothesis, category: row.category, stopReason: row.stopReason, sourceSessionId: sid });
			}

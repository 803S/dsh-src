...
export function createSrcStore(dependencies) {
	const { srcDomainSpec, sharedDomainOpens, LEGACY_KEY_MIGRATION_TABLES, recordKey, TABLE_OF_ID_KIND, snapshot, SCHEMA_OF_KIND, closeProofServersOfSession, assetGrantHosts, SEVERITIES, SRC_INFRA_DEFAULTS, SRC_INFRA_KEYS, SRC_AUTH_REQUEST_BUDGET, routePlaybook, credentialHeaders } = dependencies;
	...
	*/
	...
	async resolvePendingApproval(sessionId, id, action, note, http, runCapability, resolveCredential) {
		const table = (await this.domain()).table("pending_approvals");

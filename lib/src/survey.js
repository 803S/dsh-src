...
/**
 * crt.name provider（无需 key，国内直连实测 <2s）：apex 域名证书透明度子域清单。
 * 返回纯文本逐行一域。同 fofaSearch 注入模式：fetchImpl 注入、无网络依赖降级；纯异步不写库。
 * 返回 { ok, degraded, hosts[], roots[], note }（roots 为跨域 SAN 发现的新根域，按股权闸交用户确认，不自动入队）。
 */
export async function crtNameSearch(seed, { base = String(process.env.DSH_SRC_CRT_NAME_BASE ?? "https://crt.name/v1/search").trim(), maxHosts = 200, fetchImpl } = {}) {
	if (typeof fetchImpl !== "function") return { ok: false, degraded: true, hosts: [], roots: [], note: "crt.name fetch 未注入（测试/宿主未提供 http），跳过；种子闭环不依赖它" };
	const apex = hostOf(seed.value);
	let response;
	try {
		response = await fetchImpl(`${base}?apex=${encodeURIComponent(apex)}`, { method: "GET", headers: { "user-agent": "dsh-src-survey/1" } });
	} catch {
		return { ok: false, degraded: false, hosts: [], roots: [], note: "crt.name 请求失败，跳过" };
	}
	if (response.status !== 200) return { ok: false, degraded: false, hosts: [], roots: [], note: `crt.name 返回 ${response.status}，跳过` };
	const body = await response.text();
	const hosts = new Set();
	const roots = new Set();
	...
	return { ok: true, degraded: false, hosts: [...hosts].slice(0, maxHosts), roots: [...roots].slice(0, 20), note: `provider=crt.name, ${hosts.size} hosts` };
}

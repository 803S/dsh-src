...
/**
 * agniops provider（无需 key，国内直连实测 <2s，限流 60/min）：apex 域名子域清单。
 * [opt] 取代 crt.name 腿：实测 agniops ⊇ crt.name（example.com：agniops 4016 行、crt.name 244 行，
 * crt.name 独有仅 1 条）——crt.name 数据是其子集，保留即冗余，故整腿替换不并存。
 * 数据源为聚合库（CT 日志+DNS 枚举+历史解析），比纯 CT 覆盖面大，含日期戳类噪音（如 dns-2026…）；
 * 噪音不过滤（下游 addSurveySeed 去重 + probe 判活自然消化），只做格式清洗。
 * 返回纯文本逐行一域。同 fofaSearch 注入模式：fetchImpl 注入、无网络依赖降级；纯异步不写库。
 * 返回 { ok, degraded, hosts[], roots[], note }（agniops 不出跨域，恒空 roots；跨根域由 certspotter 腿承担）。
 */
export async function agniopsSearch(seed, { base = String(process.env.DSH_SRC_AGNIOPS_BASE ?? "https://app.agniops.in/v1/search").trim(), maxHosts = 400, fetchImpl } = {}) {
	if (typeof fetchImpl !== "function") return { ok: false, degraded: true, hosts: [], roots: [], note: "agniops fetch 未注入（测试/宿主未提供 http），跳过；种子闭环不依赖它" };
	const apex = hostOf(seed.value);
	let response;
	try {
		response = await fetchImpl(`${base}?domain=${encodeURIComponent(apex)}`, { method: "GET", headers: { "user-agent": "dsh-src-survey/1" } });
	} catch {
		return { ok: false, degraded: false, hosts: [], roots: [], note: "agniops 请求失败，跳过" };
	}
	if (response.status === 429) return { ok: false, degraded: false, hosts: [], roots: [], note: "agniops 429 限流（60/min），稍后重试" };
	if (response.status !== 200) return { ok: false, degraded: false, hosts: [], roots: [], note: `agniops 返回 ${response.status}，跳过` };
	const body = await response.text();
	const hosts = new Set();
	for (const match of body.matchAll(/[a-z0-9-]+(?:\.[a-z0-9-]+)+/gi) ?? []) {
		const host = match[0].toLowerCase().replace(/\.$/, "");
		if (host.length <= 3 || /^\d+\.\d+$/.test(host)) continue;
		if (host === apex || host.endsWith(`.${apex}`)) hosts.add(host);
	}
	return { ok: true, degraded: false, hosts: [...hosts].slice(0, maxHosts), roots: [], note: `provider=agniops, ${hosts.size} hosts` };
}

/**
 * certspotter provider（无需 key，国内直连实测可用）：证书透明度 issuances，跨域 SAN 全展开。
 * 唯一能爆集团新根域的源（同证书签发链跨根域关联）；新根域不自动入队（股权闸，交用户确认）。
 * 返回 { ok, degraded, hosts[], roots[], note }。
 */
export async function certspotterSearch(seed, { base = String(process.env.DSH_SRC_CERTSPOTTER_BASE ?? "https://api.certspotter.com/v1/issuances").trim(), maxHosts = 200, fetchImpl } = {}) {
	if (typeof fetchImpl !== "function") return { ok: false, degraded: true, hosts: [], roots: [], note: "certspotter fetch 未注入（测试/宿主未提供 http），跳过；种子闭环不依赖它" };
	const apex = hostOf(seed.value);
	let response;
	try {
		response = await fetchImpl(`${base}?domain=${encodeURIComponent(apex)}&include_subdomains=true&expand=dns_names`, { method: "GET", headers: { "user-agent": "dsh-src-survey/1" } });
	} catch {
		return { ok: false, degraded: false, hosts: [], roots: [], note: "certspotter 请求失败，跳过" };
	}
	if (response.status === 429) return { ok: false, degraded: false, hosts: [], roots: [], note: "certspotter 429 限流，稍后重试" };
	if (response.status !== 200) return { ok: false, degraded: false, hosts: [], roots: [], note: `certspotter 返回 ${response.status}，跳过` };
	const body = await response.text();
	const hosts = new Set();
	const roots = new Set();
	try {
		const parsed = JSON.parse(body);
		for (const issuance of Array.isArray(parsed) ? parsed : []) {
			for (const name of Array.isArray(issuance?.dns_names) ? issuance.dns_names : []) {
				const host = String(name).toLowerCase().replace(/\.$/, "");
				if (host.length <= 3 || !host.includes(".")) continue;
				if (host === apex || host.endsWith(`.${apex}`)) hosts.add(host);
				else roots.add(host);
			}
		}
	} catch {
		return { ok: false, degraded: false, hosts: [], roots: [], note: "certspotter 响应非 JSON，跳过" };
	}
	return { ok: true, degraded: false, hosts: [...hosts].slice(0, maxHosts), roots: [...roots].slice(0, 20), note: `provider=certspotter, ${hosts.size} hosts` };
}

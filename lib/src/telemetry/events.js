...
export const emitLayaDecision = (telemetry, exec, payload = {}) => {
	...
	const enriched = { ...payload, source: payload.source ?? "unknown", fallback: payload.fallback === true, latency: Number(payload.latency ?? 0), probabilities: payload.probabilities ?? {} };
	return telemetry.emit("laya.decision", { sessionId }, enriched);
};

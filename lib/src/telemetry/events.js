...
import { srcTelemetryFlag, srcLayaDecisionFlag } from "../flags.js";
...
/** [local.90] Laya 决策事件：仅在 shadow/on 模式下由 laya-client 上报，供影子分析与调阈值。
 * payload: {action, confidence, riskScore, method, host, path} */
export function emitLayaDecision(telemetry, exec, payload = {}) {
	if (srcLayaDecisionFlag() === "off") return "";

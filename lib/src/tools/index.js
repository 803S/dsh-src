...
export function createRegisterSrcTools(dependencies) {
	const {
		...
		classifyHttpRequest, lessonsDataDir, listAllLessons, lessonIndexLines, readLessonFile, sessionLessons, appendObservationProjection,
	...
	ctx.tools.register(defineTool({
		...
		execute: async (_args, exec) => {
			...
			const delegationState = (view.intents ?? []).map((intent) => {
				...
				if (runtime.some((entry) => entry.activity === "running")) status = "running";
				if (runtime.some((entry) => entry.activity === "inactive") && status === "not-started") status = "ready";
				const unassignedRuntimeChildren = checkpoints.length === 0 ? runtimeChildren.filter((entry) => entry.status !== "diagnostic").map((entry) => entry.id) : [];
				return { intentId: intent.id, title: intent.title, status, childSessionIds, ...(runtime.length > 0 ? { runtime } : {}), ...(unassignedRuntimeChildren.length > 0 ? { unassignedRuntimeChildren } : {}), ...(runtimeChildrenAvailable ? { runtimeObserved: true } : {}) };
			});

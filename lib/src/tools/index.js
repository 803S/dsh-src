...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_a, v) => [{
				type: "text",
				text: `Recorded asset ${v.id} [${v.type}] ${v.value}${v.edgeId === void 0 ? "" : ` (parent edge ${v.edgeId})`}${v.assetHost === void 0 ? "" : `；授权匹配 host: ${v.assetHost}（探测类工具按资产清单放行该 host，excluded 除外）`}${v.minappTodo === void 0 ? "" : `。${v.minappTodo}`}.`
			}]
		},
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			...
			/* [local.61] 数据编排第一实例：mini-program 资产进入清单 → 确定性挂「请在微信打开目标
			 * 小程序并确认登录」待办（不经模型自觉，store+合成投影事件双写，面板立即可见且重放一致）。
			 * 幂等：同 engagement 已有同前缀标题待办（任意状态）不重复；失败不阻塞资产登记。 */
			let minappTodo = void 0;
			if (args.type === "mini-program") {
				const targetId = engagementId ?? sessionId;
				try {
					const marker = "请在微信打开目标小程序";
					const existingTodo = await store.findUserTodoByTitlePrefix(targetId, marker);
					if (existingTodo !== void 0) {
						minappTodo = `小程序前置待办已存在（${existingTodo.id}，status=${existingTodo.status}），用户完成后即可跑 wx-minapp-recon 扫描线，勿重复挂同名待办`;
					} else {
						const targetSession = ctx.sessions.get(targetId);
						if (targetSession !== void 0) {
							const title = `${marker}并确认登录`;
							const detail = "目标包含小程序资产。请在微信中打开该目标小程序并完成登录（若已打开过，直接勾选完成本待办）。完成后 agent 将扫描本机 wxapkg 缓存，用已装能力 wx-minapp-recon 执行 反编译→端点/密钥提取→src_http 测试（走审批闸）。";
							await commitSyntheticMutation(targetSession, async () => {
								const todo = await store.upsertUserTodo(targetId, { title, detail, kind: "manual-test" });
								return { value: todo, events: [syntheticEvent("src_user_todo", { title, detail, kind: "manual-test" })] };
							}, appendSessionToolEvent);
							minappTodo = `已自动挂起用户待办「${title}」（数据编排触发，等同 src_user_todo，无需再提同名待办）`;
						}
					}
				} catch {}
			}

		...
		execute: async (args, exec) => {
			...
			if (layaFlags.some(f => f !== "off")) {
				...
				if (isCommander) {
					...
					// Skill Selector 决策（[local.91]）——规则层先召回候选，Laya 排序
					if (srcLayaSkillFlag() !== "off") {
						// 简化规则召回：根据 goalType 和 path 推断候选 skill
						const goalType = goal?.target?.includes("小程序") ? "mini-program" : "web";
						const candidates = goalType === "mini-program" ? ["wx-minapp-recon", "fofa"] : ["fofa", "agniops"];
						const skillLaya = await layaDecide({
							taskType: "skill-activate", goalType,
							candidates, skillMatchScores: {}, lastSkillUsed: null
						}, exec);
						if (srcLayaSkillFlag() === "shadow") {
							emitLayaDecision(telemetry, exec, { action: skillLaya.action, confidence: skillLaya.confidence, taskType: "skill", method, host: u.hostname, path: u.pathname });
						}
					}
					if (srcLayaDecisionFlag() === "shadow") {
						emitLayaDecision(telemetry, exec, { action: laya.action, confidence: laya.confidence, riskScore: laya.riskScore, method, host: u.hostname, path: u.pathname });

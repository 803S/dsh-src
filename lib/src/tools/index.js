...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		name: "src_user_todo",
		description: "创建、完成或放弃一个用户待办项——一项必须先由人工完成才能继续测试的工作（如提供已登录的 Burp 请求给认证会话、启用 Burp MCP、提供 App/小程序下载链接）。创建时给 title+detail；用户反馈完成后用同一 userTodoId 并将 status 改为 done 并附一个 note。pending/done 待办会列在 src_state.userTodos。凡完整测试被只靠人工能提供的输入阻塞时使用。【枚举纪律 [local.65]】待办只留给真正需要用户的事（登录/拍板/提供凭据/人工操作）；枚举类工作（找小程序、找 App、找关联域名、子域收集）先用自身能力做——本机微信包扫描（wx-minapp-recon）、下载反编译、测绘（fofa）、被动侦察、页面/隐私政策里的线索挖掘，穷尽这些手段拿不到才挂待办，且 detail 必须写明已尝试的手段与结果，不要把 agent 的本职工作外包给用户。",
		parameters: {
	...
	ctx.tools.register(defineTool({
		...
		const sanitizedHeaders = stripCredentialHeaders(headers); console.error("DBG sanitized=", JSON.stringify(sanitizedHeaders));

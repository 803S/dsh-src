...
import { srcLayaDecisionFlag } from "../flags.js";
...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			if (srcLayaDecisionFlag() !== "off") {
				const isCommander = exec?.agent?.session?.header?.parentSession === void 0;
				if (isCommander) {
					const laya = await layaDecide({ url: String(u), method, body, headers: rawHeaders, credentialRef }, exec);

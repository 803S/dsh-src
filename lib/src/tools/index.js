...
/* [local.81 Phase 7] 例外：survery.js 为纯函数叶子（只依赖 flags），种子闭环判定/存活分类/模式判定/FOFA 均直接 import。 */
import { hostOf as surveyHostOf, seedClosureStatus, classifyProbeResponse, surveyModeOf, fofaSearch, fofaKey } from "../survey.js";
...
export function createRegisterSrcTools(dependencies) {
	...
	if (srcSurveyFlag() !== "off") ctx.tools.register(defineTool({
		...
		parameters: {
			...
			seedId: { type: "string", description: "complete/probe/fofa：种子 id。" },
		...
		execute: async (args, exec) => {
			...
			if (action === "probe") {
				...
				for (const host of hosts) {
					...
					if (outcome === "alive") await store.addAsset(sessionId, { type: "subdomain", value: host, source: "survery-probe", status: "candidate", confidence: .6 });
					if (outcome === "dead") {
						const data = await store.sessionData(sessionId);
						const intentId = data.intents[0]?.id;
						if (intentId !== void 0) await store.addFact(sessionId, { intentId, kind: "info", target: host, detail: "非存活：超时/探不通（src_survey_seed prode，防重复探测）", confidence: .6 });
					}
				}

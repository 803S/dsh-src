...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			/* [local.87] artifacts 约定：建 goal 起手建齐 $DSH_HOME/artifacts/{engagement}/ 五子夹+README
			 * （幂等，已存在跳过）；artifactsDir 返回给 agent 作默认落盘路径（散落都是因为现在没有默认）。
			 * README 是文件不是上下文，不进状态预算；scaffold 失败不阻塞 goal 创建。 */
			let artifactsDir;
			try {
				await ensureArtifactsScaffold(sessionId, { target: goal.target, goalId: goal.id });
				artifactsDir = artifactsRoot(sessionId);

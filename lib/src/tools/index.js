...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		if (exec.agent !== void 0) appendSessionToolEvent(exec.agent.session, "src_record_pending_approval", { id: pending.id, method, url: String(u), path: u.pathname, headers: headersText, body: storedBody, category: verdict.category, reason: verdict.reason, justification, ...(credentialRef !== "" ? { credentialRef } : {}) });

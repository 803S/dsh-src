...
export function createRegisterSrcTools(dependencies) {
	const {
		...
		assetGrantHosts, assetGrantHostsFor, hostCoveredByAssets, parseGoalHost, isPlausiblePublicHost, str, dshHomeOf, readCapsManifest, readCoverageChecklist, capsWiredIds,
	...
	ctx.tools.register(defineTool({
		...
		output: {
			schema: {
				...
				properties: {
					...
					coverageTodos: { type: "array", items: { type: "string" } },
			...
			render: (_a, v) => {
				...
				/* [local.63] 覆盖面编排：开局确定性的资产类枚举待办（小程序/App/关联主体/海外）。 */
				if (Array.isArray(v.coverageTodos) && v.coverageTodos.length > 0) {
					lines.push("", `【覆盖面编排】${v.coverageTodos.length} 张覆盖待办已挂（数据编排，与主站线并行推进，勿因等待用户而停工）：`, ...v.coverageTodos.map((l) => `- ${l}`));
				}
		...
		execute: async (args, exec) => {
			 ...
			 * 数据来自 preset/src-hunter/coverage.yaml（运行时可用 ~/.dsh/coverage.yaml 覆盖）。待办不阻塞 agent，
			 * 枚举与主站线并行；幂等靠 findUserTodoByTitlePrefix（同 engagement 同标题任意状态不重复）；
			 * 失败不阻塞建 goal。规则说不收录的类别也走待办确认（不机械拦截）——覆盖面待办是数据，不是闸门。 */
			let coverageTodos;
			try {
				const checklistItems = await readCoverageChecklist(dshHomeOf());
				const covLines = [];
				for (const item of checklistItems) {
					const covTitle = String(item.title).replaceAll("{target}", goal.target);
					const existingTodo = await store.findUserTodoByTitlePrefix(sessionId, covTitle);
					if (existingTodo !== void 0) {
						covLines.push(`覆盖面待办已存在（${existingTodo.id}，status=${existingTodo.status}）：${covTitle}`);
						continue;
					}
					const covDetail = String(item.detail ?? "").replaceAll("{target}", goal.target);
					const covKind = typeof item.kind === "string" ? item.kind : "other";
					await commitSyntheticMutation(exec.agent?.session, async () => {
						const todo = await store.upsertUserTodo(sessionId, { title: covTitle, detail: covDetail, kind: covKind });
						return { value: todo, events: [syntheticEvent("src_user_todo", { id: todo.id, title: covTitle, detail: covDetail, kind: covKind })] }; /* [local.64] 带真实 store id，防投影漂移 */
					}, appendSessionToolEvent);
					covLines.push(`已挂覆盖面待办「${covTitle}」（${item.id}，数据编排）`);
				}
				if (covLines.length > 0) coverageTodos = covLines;
			} catch { coverageTodos = void 0; }
			return {
				id: goal.id,
				target: goal.target,
				objective: goal.objective,
				...(lessonIndex.length > 0 ? { lessonIndex } : {}),
				...(coverageTodos !== void 0 ? { coverageTodos } : {}),
						...
						if (existingTodo !== void 0) {
							...
							if (targetSession !== void 0) {
								...
								await commitSyntheticMutation(targetSession, async () => {
									...
									return { value: todo, events: [syntheticEvent("src_user_todo", { id: todo.id, title: todoTitle, detail, kind })] }; /* [local.64] 带真实 store id */
								}, appendSessionToolEvent);
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_a, v) => {
				...
				const assetGapsNote = view.assetGaps === void 0 ? "" : (view.assetGaps.gaps.length > 0 ? ` 资产类缺口：${view.assetGaps.gaps.join("、")}（先枚举登记再谈测完，对应覆盖面待办）。` : ` 资产类覆盖：核心四类全有登记。`);
				return [{
					type: "text",
		...
		execute: async (_args, exec) => {
			 ...
			 * 「图里没有」不等于「世界里没有」，缺口类先枚举登记（对应覆盖面待办）再谈测完。 */

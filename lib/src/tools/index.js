...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: (_a, v) => [{ type: "text", text: (v.approval === "allowed" || v.approval === "allowed-auto" ? `src_http ${v.status} ${v.method} ${v.path}（已执行）` : v.approval === "pending" ? `src_http 已挂起待审 ${v.pendingApprovalId ?? ""}（未发出）。${v.reason ?? ""}` : `src_http 未执行：审批 ${v.approval}——${v.reason ?? ""}`) + (v.skillHint !== void 0 && v.skillHint !== "" ? `\n${v.skillHint}` : "") }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const engagementId = await resolveEngagementSession(store, ctx, exec);
			const goal = engagementId === void 0 ? void 0 : await store.getGoal(engagementId);
			...
			if (layaFlags.some(f => f !== "off")) {
				...
				if (isCommander) {
					...
					/* [Bug B] choice 的 confidence 是熵值（≤0.82），阈值必须用 probabilities[action] */
						...
						if (laya.action === "allow") {
							const httpT0 = Date.now();
							await srcHttpThrottle(engagementId ?? sessionId);
							const response = await http(u, { method, redirect: "manual", headers: { "user-agent": "dsh-src/1", ...rawHeaders }, ...(body !== void 0 ? { body } : {}) });
							let rawBodyText = "";
							try { rawBodyText = await response.text(); } catch {}
							const described = describeHttpBody({ url: String(u), method, status: response.status, contentType: response.headers?.get?.("content-type") ?? "", rawBody: rawBodyText, secrets: [], full: args.full === true });
								/* [响应体语义分类] shadow=只落 telemetry；on=可覆盖 protectionSignal（waf-block）。 */
								const respCls = await classifyResponse(exec, telemetry, response, rawBodyText, u, method);
							tel(exec, "http.request", { host: u.hostname, path: u.pathname, method, status: response.status, durationMs: Date.now() - httpT0, responseBytes: rawBodyText.length, approval: "allowed-auto" }, engagementId ?? sessionId);
							return { method, path: u.pathname, status: response.status, approval: "allowed-auto", reason: verdict.reason, ...(contentTypeAutoAdded ? { contentTypeAutoAdded } : {}), ...(respCls.responseClass !== "" ? { responseClass: respCls.responseClass } : {}), ...(respCls.leakSignal ? { leakSignal: true } : {}), responseBody: described.body, ...(described.truncated ? { truncated: true } : {}), ...(described.deduped ? { deduped: true } : {}), ...skillHint !== "" ? { skillHint } : {} };
						}
		...
		output: {
			...
			render: (_a, v) => {
				...
				if (v.priorContext && Object.keys(v.priorContext).length > 0) {
					...
					if (Array.isArray(v.priorContext.findings) && v.priorContext.findings.length > 0) lines.push(`已确认漏洞（复用/回归）：`, ...v.priorContext.findings.map((f) => `- [${f.severity}${f.status && f.status !== "active" ? "·" + f.status : ""}] ${f.title}`));

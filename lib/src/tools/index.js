	...
	async function classifyResponse(exec, telemetry, response, rawBodyText, u, method) {
		...
		try {
			...
			emitLayaDecision(telemetry, exec, {
				action: out.responseClass, confidence: respLaya.confidence,
				...
				if (isCommander) {
					...
					if (srcLayaDelegateFlag() !== "off") {
						...
						if (srcLayaDelegateFlag() === "shadow") {
							emitLayaDecision(telemetry, exec, { action: delegateLaya.action, confidence: delegateLaya.confidence, taskType: "delegate", method, host: u.hostname, path: u.pathname });
						}
					}
					...
					if (srcLayaDecisionFlag() === "shadow") {
						emitLayaDecision(telemetry, exec, { action: laya.action, confidence: laya.confidence, riskScore: laya.riskScore, taskType: "risk-grade", method, host: u.hostname, path: u.pathname });

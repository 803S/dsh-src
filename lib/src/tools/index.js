...
import { srcStateVersionFlag, srcOrchestratorFlag, srcEventStoreFlag } from "../flags.js";
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			if (auditedCoverage.length > 0) {
				const observedPaths = new Set((view.observations ?? []).map((o) => String(o.path ?? "")).filter((p) => p !== ""));
				for (const row of auditedCoverage) {
					...
					if (claimed > observedPaths.size) {
						blockers.push(`coverage ${row.id}（${row.phase}/${row.category}）声明实测 ${claimed} 个接口，但 observations 里去重 path 仅 ${observedPaths.size} 个——实测数与请求记录对不上，请补 src_record_observation 或如实下调 endpointsTested`);
					}

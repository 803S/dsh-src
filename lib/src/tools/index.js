...
import { srcStateVersionFlag } from "../flags.js";
...
export function createRegisterSrcTools(dependencies) {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const routed = routePlaybook(args.title, args.detail ?? "");
	...
	ctx.tools.register(defineTool({
		...
		execute: async (_args, exec) => {
			...
			if (detailMode === "orchestration") return { version: 2, view: "orchestration", initialized: view.initialized, goal: goal2, counts: view.counts, intents: (view.intents ?? []).map((intent) => ({ id: intent.id, title: intent.title, status: intent.status, priority: intent.priority ?? 5 })), orphanIntents, delegationState, runtimeChildren, unassignedRuntimeChildren, pendingApprovals: view.pendingApprovals ?? [], userTodos: view.userTodos ?? [] };

...
import { hostOf as surveyHostOf, seedClosureStatus, classifyProbeResponse, surveyModeOf, fofaSearch, agniopsSearch, certspotterSearch, fofaKey } from "../survey.js";
/* [local.87] 例外：artifacts.js 为文件 IO 叶子（只依赖 node:fs/os/path，不碰 domain/projection/store），直接 import。 */
import { ensureArtifactsScaffold, artifactsRoot } from "../artifacts.js";
/* [local.90] 例外：laya-client.js 为 HTTP 客户端叶子（只依赖 node:fetch，不碰 domain/projection/store），直接 import。 */
import { layaDecide } from "../decision/laya-client.js";
/* [local.94] 例外：skill-recall.js 为纯函数叶子（只依赖 playbooks.js，不碰 domain/store），直接 import。 */
import { recallCandidates, skillReminderLine, createSkillReminderRegistry } from "../decision/skill-recall.js";
import { patternOf, patternShapeError, PATTERN_COLUMNS } from "../lessons.js";
import { srcLayaDecisionFlag, srcLayaDelegateFlag, srcLayaSkillFlag } from "../flags.js";
import { emitLayaDecision } from "../telemetry/events.js";
import { clip, requestSecrets, sanitizeEvidence, safeHeaders, safeRequestBody, renderHttp, renderScan, renderBypass, renderDecisionView } from "../evidence-output.js";
import { createHttpBodyDescriber } from "../http-output.js";

/* [local.94] Skill 提醒去重注册表（纯内存，dsh 重启重置）。按会话维度记「已提醒 / 已读过」。 */
const skillReminders = createSkillReminderRegistry();
...
export function createRegisterSrcTools(dependencies) {
	const {
		...
		classifyHttpRequest, lessonsDataDir, listAllLessons, lessonIndexLines, readLessonFile, sessionLessons, lessonsForContext, patternLessonsForContext, lessonHintLines, appendObservationProjection,
	...
	// Every executed request leaves evidence, independently of model-facing deduplication.
	async function captureHttpEvidence(exec, engagementId, { url, method, headers, body, response, rawBody, intentId, assetId, readError = false }) {
		const secrets = requestSecrets(headers);
		const cleanUrl = sanitizeEvidence(String(url), secrets);
		const responseHeaders = safeHeaders(response.headers, secrets);
		const saved = createHttpBodyDescriber()({ url: cleanUrl, method, status: response.status, contentType: response.headers?.get?.("content-type") ?? "", rawBody: sanitizeEvidence(rawBody, secrets), secrets, full: true });
		const common = { url: cleanUrl, responseHeaders };
		try {
			const record = await store.upsertObservation(engagementId, {
				...(intentId ? { intentId } : {}), ...(assetId ? { assetId } : {}),
				method, path: cleanUrl, httpStatus: response.status,
				reqHeaders: safeHeaders(headers, secrets), reqBodySnippet: safeRequestBody(body, secrets),
				respHeaders: responseHeaders, respBodySnippet: clip(saved.body, 32000), protectionSignal: [403, 429, 503].includes(response.status),
				source: "raw", decision: `src_http 自动留证（脱敏、有界）${readError ? "；响应体读取失败，不能推断为空响应" : ""}`
			});
			appendSessionToolEvent(ctx.sessions.get(engagementId) ?? exec.agent?.session, "src_record_observation", record);
			tel(exec, "evidence.created", { evidenceType: "observation", evidenceId: record.id, status: record.httpStatus, source: "src_http" }, engagementId);
			return { ...common, evidenceId: record.id, ...(readError ? { evidenceError: "请求已执行，但响应体读取失败；仅保存状态与响应头，不要盲目重放写操作。" } : {}) };
		} catch {
			return { ...common, evidenceError: "请求已执行，但证据落库或投影同步失败；请保存当前响应，不要盲目重放写操作。" };
		}
	}
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: renderScan },
	...
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: renderBypass },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			/* [local.17] 委派子代理继承对话上下文但不继承 store 行：沿 parentSession 链找到持有 goal 的会话再用。 */
			const engagementId = await resolveEngagementSession(store, ctx, exec);
	...
	ctx.tools.register(defineTool({
		...
		description: "补录外部 HTTP 探测或导入流量的观察（时间线/证据层）。src_http 已自动留证并返回 evidenceId，不要重复抄录；外部 Burp/HAR 等未自动落库的流量用本工具补录真实响应与决策。有界：只记状态码、响应片段(<=2000字符)、是否撞 WAF/403/429（401 是认证边界不是风控）、是否绕过尝试成功。",
		parameters: {
	...
	ctx.tools.register(defineTool({
			...
			assets: {
				...
				items: {
					...
					properties: {
						...
						source: {
							type: "string",
							description: "资产来源（agniops/certspotter/FOFA/DNS/官网导航/JS bundle/手工确认等），必须填写以保留溯源。"
						},
						method: {
							type: "string",
		...
		output: {
			...
			render: (_a, value) => [{
				type: "text",
				text: `Submitted ${value.facts} facts, ${value.assets} assets, and ${value.findings} findings to the parent session.\n按输入顺序（含去重项）：facts=${(value.factIds ?? []).join(",")} assets=${(value.assetIds ?? []).join(",")} findings=${(value.findingIds ?? []).join(",")}\ncheckpoint=${value.checkpointId} stage=${value.stage}；按这些 ID 用 src_get_evidence 读取父任务证据。`
			}]
		},
		execute: async (args, exec) => {
			...
			const factIds = [], assetIds = [], findingIds = [];
			try {
			for (const write of factWrites) {
				const result = await store.addFact(parentSessionId, write);
				factIds.push(result.nodeId);
				if (!result.duplicate) { appendSessionToolEvent(parent, "src_add_fact", write); acceptedFacts.push(write); tel(exec, "evidence.created", { evidenceType: "fact", evidenceId: result.nodeId ?? "", intentId, kind: write.kind, source: "src_submit" }, parentSessionId); }
			}
			for (const write of assetWrites) {
				const result = await store.addAsset(parentSessionId, write);
				assetIds.push(result.nodeId);
				if (!result.duplicate) { appendSessionToolEvent(parent, "src_add_asset", write); acceptedAssets.push(write); }
			}
			for (const write of findingWrites) {
				const result = await store.addFinding(parentSessionId, write);
				findingIds.push(result.nodeId);
				if (!result.duplicate) { appendSessionToolEvent(parent, "src_add_finding", write); acceptedFindings.push(write); tel(exec, "evidence.linked", { sourceId: intentId, targetId: result.nodeId ?? "", relation: "proves", severity: write.severity, source: "src_submit" }, parentSessionId); }
			}
			} catch (error) {
				throw new Error(`${error.message}\n已持久化（不要猜 ID）：facts=${factIds.join(",")} assets=${assetIds.join(",")} findings=${findingIds.join(",")}；修正失败项后重试，已写项会去重。`);
			}
			...
			if (!checkpoint.duplicate) appendSubmissionProjection(parent, intentId, [], [], [], { id: checkpoint.id, childSessionId, stage, summary, facts: acceptedFacts.length, assets: acceptedAssets.length, findings: acceptedFindings.length, createdAt: checkpoint.createdAt, batchKey }, optionalString(input.decision));
			return {
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const sessionId = await resolveEngagementSession(store, ctx, exec) ?? callerId;
			if (callerId !== sessionId) {
				const data = await store.sessionData(sessionId);
				if (!data.checkpoints.some((c) => c.childSessionId === callerId && c.intentId === args.intentId)) throw new Error("子代理需先用 src_submit 向此 intent 提交检查点，再更新对应研究记录");
				const refs = new Set([...data.facts, ...data.observations, ...data.research].filter((r) => r.intentId === args.intentId).map((r) => r.id));
				if (!Array.isArray(args.evidence) || args.evidence.length === 0 || args.evidence.some((id) => !refs.has(id))) throw new Error("子代理研究 evidence 必须引用该 intent 下已存在的 fact/observation/research ID");
				if (args.findingId && !data.findings.some((f) => f.id === args.findingId && f.intentId === args.intentId)) throw new Error("子代理 findingId 必须属于同一 intent");
			}
			...
			if (callerId !== sessionId) appendSessionToolEvent(ctx.sessions.get(sessionId), "src_record_research", record);
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			for (const rel of candidates) {
				...
				const MAX = 12000;
				/* [local.94] Skill 提醒去重：读过的 skill 不再提醒。 */
				try { skillReminders.markRead(sessionIdOf(exec), id); } catch {}
			...
			/* [local.90] 目录内无任何候选文档时，列出目录实际内容帮助模型/用户定位（如纯脚本型能力可 file 指定脚本 --help 或提示改用 src_list_capabilities）。 */
			let listing = "";
			try { listing = (await fsPromises.readdir(nodePath.resolve(item.dir))).slice(0, 10).join(", "); } catch {}
			throw new Error(`src_read_capability: 能力 ${id} 目录内未找到文档（试过：${candidates.join("、")}；目录现有：${listing || "（空）"}）。可用 file 参数指定目录内其他文件，或该能力无文档时直接看 scripts 白名单与 when 描述（src_list_capabilities）用法后再 src_run_capability`);
		}
	}));
	ctx.tools.register(defineTool({
	...
	/* [响应体语义分类] src_http 放行后对响应体调一次 Laya choice 分类。
	 * shadow=只落 telemetry；on=可覆盖 protectionSignal（waf-block 判定）。
	 * 仅 commander 生效；Laya 不可达时静默（不阻塞放行路径）。 */
	async function classifyResponse(exec, telemetry, response, rawBodyText, u, method) {
		const out = { responseClass: "", leakSignal: false };
		if (srcLayaDecisionFlag() === "off") return out;
		if (exec?.agent?.session?.header?.parentSession !== void 0) return out;
		try {
			const respLaya = await layaDecide({
				taskType: "response-classify",
				status: response.status,
				contentType: response.headers?.get?.("content-type") ?? "",
				bodySnippet: rawBodyText.slice(0, 1024),
			}, exec);
			out.responseClass = respLaya.action || "";
			/* noul 的 confidence=P(true)（可直接当概率用）；优先取 daemon 返回的 leak.noul 字段，缺省视为无泄露。 */
			out.leakSignal = (respLaya.leak?.noul ?? respLaya.leak?.confidence ?? 0) >= 0.7;
			emitLayaDecision(telemetry, exec, {
				action: out.responseClass, confidence: respLaya.confidence, probabilities: respLaya.probabilities, source: respLaya.source, fallback: respLaya.fallback, latency: respLaya.latency,
				taskType: "response-classify", method, host: u.hostname, path: u.pathname,
				status: response.status, leak: out.leakSignal,
			});
		} catch {}
		return out;
	}
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: renderHttp },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const engagementId = await resolveEngagementSession(store, ctx, exec);
			const goal = engagementId === void 0 ? void 0 : await store.getGoal(engagementId);
			if (goal === void 0) throw new Error("src_http requires an initialized SRC goal");
			if (args.intentId) await store.requireRef(engagementId, "intents", args.intentId, "intent");
			if (args.assetId) await store.requireRef(engagementId, "assets", args.assetId, "asset");
			const http = makeHttpFetch(await store.getInfra(engagementId ?? sessionId));
			const target = new URL(`https://${parseGoalHost(goal.target)}`);
			...
			const credentialSecretValues = [credentialSecret, ...requestSecrets(headers)].filter(Boolean);
			...
			/* [local.90] Laya 自动审批闸：commander 会话且 flag 非 off 时，先经 Laya 决策。
			   影子模式只记录不干预；正式模式按置信度自动放行/拦截；守护进程不可达则 fail-open。 */
			const layaFlags = [srcLayaDecisionFlag(), srcLayaDelegateFlag(), srcLayaSkillFlag()];
			/* [local.94] Skill 提醒注入（on 模式）：shadow=只记录 telemetry；on=过闸后附 ≤1 行到返回值。 */
			let skillHint = "";
			if (layaFlags.some(f => f !== "off")) {
				const isCommander = exec?.agent?.session?.header?.parentSession === void 0;
				if (isCommander) {
					// 风险审批（已有）
					const laya = await layaDecide({ url: String(u), method, body, headers: rawHeaders, credentialRef, taskType: "risk-grade" }, exec);
					// Delegation 决策（[local.91]）
					if (srcLayaDelegateFlag() !== "off") {
						const delegateLaya = await layaDecide({
							url: String(u), method, body, headers: rawHeaders, credentialRef,
							taskType: "delegate",
							estimatedSteps: 3, requiresIsolation: false, contextDependency: "medium"
						}, exec);
						if (srcLayaDelegateFlag() === "shadow") {
							emitLayaDecision(telemetry, exec, { action: delegateLaya.action, confidence: delegateLaya.confidence, probabilities: delegateLaya.probabilities, source: delegateLaya.source, fallback: delegateLaya.fallback, latency: delegateLaya.latency, taskType: "delegate", method, host: u.hostname, path: u.pathname });
						}
					}
					// Skill Selector 决策（[local.94] 定位修正：提醒器，非激活器）——
					// 规则层用 scoreRoutes 预筛（真实分数+matchedBy），无正分候选则静默跳过；Laya 只做语义排序。
					if (srcLayaSkillFlag() !== "off") {
						const sid = sessionIdOf(exec);
						const skillCtx = { method, path: u.pathname, query: u.search, body, category: verdict.category, goalTarget: goal?.target ?? "" };
						const skillCandidates = recallCandidates(skillCtx, 5);
						if (skillCandidates.length > 0) {
							const skillLaya = await layaDecide({
								taskType: "skill-activate",
								method, path: u.pathname, query: u.search, goalTarget: goal?.target ?? "",
								candidates: skillCandidates,
								lastSkillUsed: null
							}, exec);
							/* 污染闸：①非 skip ②confidence ≥ 0.6 ③本会话未提醒过 ④本会话未读过 */
							const picked = skillCandidates.find((c) => c.id === skillLaya.action);
							const passed = srcLayaSkillFlag() === "on" && picked !== void 0 &&
								skillLaya.confidence >= 0.6 &&
								skillReminders.canRemind(sid, picked.id) &&
								!skillReminders.isRead(sid, picked.id);
							if (passed) skillReminders.markReminded(sid, picked.id);
							/* [Phase 1 telemetry] shadow/on 均落 telemetry（不进 agent 上下文）。 */
							emitLayaDecision(telemetry, exec, {
								action: skillLaya.action, confidence: skillLaya.confidence,
								probabilities: skillLaya.probabilities, source: skillLaya.source, fallback: skillLaya.fallback, latency: skillLaya.latency,
								taskType: "skill", method, host: u.hostname, path: u.pathname,
								candidates: skillCandidates.map((c) => c.id),
								reminded: passed
							});
							if (passed) skillHint = skillReminderLine(picked);
						}
					}
					if (srcLayaDecisionFlag() === "shadow") {
						emitLayaDecision(telemetry, exec, { action: laya.action, confidence: laya.confidence, riskScore: laya.riskScore, probabilities: laya.probabilities, source: laya.source, fallback: laya.fallback, latency: laya.latency, taskType: "risk-grade", method, host: u.hostname, path: u.pathname });
					/* [Bug B] choice 的 confidence 是熵值（≤0.82），阈值必须用 probabilities[action] */
					} else if (srcLayaDecisionFlag() === "on" && (laya.probabilities?.[laya.action] ?? 0) >= 0.85) {
						// Laya allow is advisory only: rules requiring approval always win.
						// Use the single executor below, including injected headers and evidence capture.
						if (laya.action === "reject") {
							return { method, path: u.pathname, status: 0, approval: "rejected", reason: `Laya auto-reject (risk=${laya.riskScore.toFixed(1)}, confidence=${laya.confidence.toFixed(2)})` };
						}
						/* 置信度不足或建议 pending → 继续走原有 classifyHttpRequest 流程 */
					}
				}
			}
			if (!verdict.require) {
				 ...
				 * Content-Type 假阴性的直接代价。截断/去重/掩码统一走 describeHttpBody。 */
								let rawBodyText = "", readError = false;
								try { rawBodyText = await response.text(); } catch { readError = true; }
								const evidence = await captureHttpEvidence(exec, engagementId ?? sessionId, { url: u, method, headers, body, response, rawBody: rawBodyText, intentId: args.intentId, assetId: args.assetId, readError });
								/* [响应体语义分类] shadow/on 均过分类（shadow 只落 telemetry），on 模式可覆盖 protectionSignal */
								const respCls = await classifyResponse(exec, telemetry, response, rawBodyText, u, method);
								const described = describeHttpBody({ sessionId: engagementId ?? sessionId, url: String(u), method, status: response.status, contentType: response.headers?.get?.("content-type") ?? "", rawBody: sanitizeEvidence(rawBodyText, credentialSecretValues), secrets: credentialSecretValues, full: args.full === true });
								...
								return { ...evidence, method, path: sanitizeEvidence(u.pathname, credentialSecretValues), status: response.status, approval: "allowed-auto", reason: verdict.reason, ...(contentTypeAutoAdded ? { contentTypeAutoAdded } : {}), ...(respCls.responseClass !== "" ? { responseClass: respCls.responseClass } : {}), ...(respCls.leakSignal ? { leakSignal: true } : {}), responseBody: described.body, ...(described.truncated ? { truncated: true } : {}), ...(described.deduped ? { deduped: true } : {}), ...skillHint !== "" ? { skillHint } : {} };
		}
		...
		const pending = await store.addPendingApproval(engagementId ?? sessionId, { ...(args.intentId ? { intentId: args.intentId } : {}), method, url: String(u), path: u.pathname, headers: headersText, ...(credentialRef !== "" ? { credentialRef } : {}), body: storedBody, category: verdict.category, reason: verdict.reason, justification });
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_a, v) => {
				...
				if (v.priorContext && Object.keys(v.priorContext).length > 0) {
					...
					if (Array.isArray(v.priorContext.blockedHypotheses) && v.priorContext.blockedHypotheses.length > 0) lines.push(`历史受阻（未证伪，条件变化可继续）：`, ...v.priorContext.blockedHypotheses.slice(0, 10).map((h) => `- [${h.category}] ${h.hypothesis}（${h.stopReason || "未记录理由"}）`));
					if (Array.isArray(v.priorContext.findings) && v.priorContext.findings.length > 0) lines.push(`历史 finding（含打回记录，使用前复核）：`, ...v.priorContext.findings.map((f) => `- [${f.severity}${f.status && f.status !== "active" ? "·" + f.status : ""}] ${f.title}`));
				/* [local.86] Dangling items from prior sessions: pending/abandoned todos + unfinished intents. */
				if (Array.isArray(v.priorContext.pendingItems) && v.priorContext.pendingItems.length > 0) lines.push(`悬空项（上轮未收尾，优先处理或明确放弃）：`, ...v.priorContext.pendingItems.map((p) => `- [${p.kind}·${p.status}] ${p.title}（${p.id}）`));
				if (v.priorContext.pendingItemsTruncated) lines.push("- （悬空项超过 10 条已截断）");
				}
				...
				/* [local.87] 文件类产物默认落盘路径：下载 JS/测绘导出/探测输出按约定进 artifacts 目录（README 有说明），不再散落工程根。 */
				if (typeof v.artifactsDir === "string") lines.push("", `【文件类产物目录】${v.artifactsDir}（五子夹 js/recon/probe/apks/报告 + README；下载的 JS、测绘导出、探测输出按类入夹，不散落工程根；结构化数据仍在库里）。`);
		...
		execute: async (args, exec) => {
			...
			/* [local.87/89] artifacts 约定：建 goal 起手建齐 $DSH_HOME/artifacts/{主域}/{session}/ 五子夹+README
			 * （主域夹同域多会话归拢；旧扁平会话夹自动迁移；幂等，已存在跳过）；artifactsDir 返回给 agent
			 * 作默认落盘路径（散落都是因为现在没有默认）。README 是文件不是上下文，不进状态预算；
			 * scaffold 失败不阻塞 goal 创建。 */
			let artifactsDir;
			try {
				await ensureArtifactsScaffold(sessionId, { target: goal.target, goalId: goal.id });
				artifactsDir = artifactsRoot(sessionId, { target: goal.target });
			} catch { artifactsDir = void 0; }
			return {
				id: goal.id,
				target: goal.target,
				...
				...(artifactsDir !== void 0 ? { artifactsDir } : {})
			};
		}
	}));
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_a, v) => [{
				type: "text",
				text: `Recorded intent ${v.id}「${v.title}」 (${v.edgeKind} ${v.sourceId} → ${v.id}, edge ${v.edgeId}).\n文档：${(v.playbook?.docs ?? []).join(", ")}\n首轮检查：\n${(v.playbook?.checks ?? []).map((s) => clip(s, 700)).join("\n")}${v.lessonHints === void 0 ? "" : `\n${v.lessonHints.join("\n")}`}`
			}]
		},
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			...
			let lessonHints;
			try {
				const context = { tool: "src_add_intent", text: `${args.title ?? ""} ${args.detail ?? ""}`, target: String((await store.getGoal(sessionId))?.target ?? "") };
				lessonHints = lessonHintLines(await lessonsForContext(context));
				const patternHits = await patternLessonsForContext(context);
				if (patternHits.length > 0) lessonHints = [...(lessonHints ?? []), ...patternHits.map((hit) => `【Pattern 形态命中】${hit.title}（src_read_lesson id=${JSON.stringify(hit.file)}；${hit.reason}）——先读全文，按四列验证并保留假点对照`)].slice(0, 5);
			} catch {}
			return {
				id: write.nodeId,
				title: args.title,
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			source: {
				type: "string",
				required: true,
				description: "发现/归属判定来源与依据（必填，审计线索：agniops/certspotter/FOFA/DNS/JS 提取/品牌归属判断等）。"
			},
			parentId: {
				type: "string",
	...
	ctx.tools.register(defineTool({
		...
		output: {
			schema: {
				...
				properties: {
					...
					goal: {
						/* [local.90] 未建 goal 时 v2 视图返回 null；dsh-tools schema 不支持 type 数组，用 oneOf 表达 nullable。 */
						oneOf: [{ type: "object", additionalProperties: true, properties: {} }, { type: "null" }]
					},
					counts: {
						type: "object",
						additionalProperties: true,
			...
			render: (_a, v) => {
				...
				if (view.version === 2) return renderDecisionView(view);
		...
		execute: async (_args, exec) => {
			const sessionId = await resolveEngagementSession(store, ctx, exec) ?? sessionIdOf(exec);
			 ...
			 * 事实分级：summary 给 evidenceIndex 索引（recent 12 + observations 12 + byIntent 计数）；
			...
			const evidenceIndex2 = { recent: recentFacts2.map((fact) => fact.id), observations: (view.observations ?? []).slice(-12).map((o) => o.id), byIntent: byIntent2, omitted: Math.max(0, allFacts2.length - RECENT_EVIDENCE_LIMIT) };
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			ids: { type: "array", required: true, items: { type: "string" }, description: "证据 id 列表（包括 observation-N），最多32个；总正文预算默认16KB/full时64KB，超预算缩小ids继续读。" },
			offset: { type: "integer", description: "正文字符偏移，默认0。截断后用返回的 nextOffset 对单个id续读，无需重发请求。" },
			full: { type: "boolean", description: "true 时单条上限放宽到 32KB（默认 4KB 截断）。" }
		},
		output: { schema: { type: "object", additionalProperties: true, properties: {} }, render: (_a, v) => [{ type: "text", text: (v.items ?? []).map((row) => row.found ? `${row.id}（${row.kind}）${row.truncated ? `［已截断，单独读此id offset=${row.nextOffset}］` : ""}：${row.content}` : `${row.id}：未找到（不在本会话图内）`).join("\n---\n") }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const engagementId = await resolveEngagementSession(store, ctx, exec) ?? sessionId;
			const view = await store.view(engagementId);
			const nodes = [];
			...
			pushNodes("obs", view.observations, (o) => `${o.method} ${o.path} → ${o.httpStatus}${o.protectionSignal ? "（防护信号）" : ""}\n${o.decision ?? ""}\n=== Request headers/body（脱敏） ===\n${o.reqHeaders ?? ""}\n${o.reqBodySnippet ?? ""}\n=== Response ===\n${o.respHeaders ?? ""}\n${o.respBodySnippet ?? ""}`.trim());
			...
			let remainingChars = args.full === true ? 64000 : 16000;
			...
			const items = (Array.isArray(args.ids) ? args.ids : []).slice(0, 32).map((rawId) => {
				...
				const content = text.slice(offset, offset + Math.min(limit, remainingChars));
				remainingChars -= content.length;
				const nextOffset = Math.min(text.length, offset + content.length);
				const truncated = nextOffset < text.length;
				return { id, found: true, kind: entry.kind, content, truncated, nextOffset, totalChars: text.length };
			});
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const evidenceExists = (id) => {
				...
				if ((view.observations ?? []).some((n) => n.id === id)) return true;
			...
			try {
				const context = { tool: "src_finalize_engagement", text: `收官报告 ${view.goal?.target ?? ""}`, target: String(view.goal?.target ?? "") };
				finalizeLessonHints = lessonHintLines(await lessonsForContext(context));
				const patternHits = await patternLessonsForContext(context);
				if (patternHits.length > 0) finalizeLessonHints = [...(finalizeLessonHints ?? []), ...patternHits.map((hit) => `【Pattern 形态命中】${hit.title}（src_read_lesson id=${JSON.stringify(hit.file)}；${hit.reason}）——检查报告是否保留假点与匿名/认钥对照`)].slice(0, 5);
			} catch {}
	...
	ctx.tools.register(defineTool({
		...
		output: { schema: { type: "object", additionalProperties: false, properties: { id: { type: "string", required: true }, status: { type: "string", required: true }, method: { type: "string", required: true }, url: { type: "string", required: true }, responseStatus: { type: "number", required: true }, note: { type: "string", required: true }, runOutput: { type: "string" }, responseBody: { type: "string" }, evidenceId: { type: "string" }, evidenceError: { type: "string" }, responseHeaders: { type: "string" } } }, render: (_a, v) => [{ type: "text", text: v.status === "approved" ? (v.method === "RUN" ? `已批准并执行 ${v.url}（exit=${v.responseStatus}）。脚本输出：\n${v.runOutput ?? "（无输出）"}` : `已批准并发出待审请求 ${v.id}：${v.method} ${v.url} → 响应 ${v.responseStatus}。${v.evidenceId ? `证据 ${v.evidenceId}（src_get_evidence 可读）。` : ""}${v.evidenceError ?? ""}\n${v.responseHeaders ?? ""}\n据响应推进（成功则记录证据/产 finding；失败则转其他方向）。${v.responseBody !== void 0 ? `\n响应体：\n${v.responseBody}` : ""}`) : `已拒绝待审请求 ${v.id}：${v.method} ${v.url} 不发出。备注：${v.note}。转其他方向。` }] },
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			const id = requiredString(args.id, "id");
			...
			const throttledHttp = async (url, init) => {
				...
				let textPromise;
				return { status: response.status, headers: response.headers, text: () => (textPromise ??= (async () => {
					let rawBody = "", readError = false;
					try { rawBody = await response.text(); } catch { readError = true; }
					replayEvidence = await captureHttpEvidence(exec, engagementId ?? sessionId, { url, method: init.method, headers: init.headers, body: init.body, response, rawBody, intentId: selfApproval?.intentId, readError });
					return sanitizeEvidence(rawBody, requestSecrets(init.headers));
				})()) };
			};
			...
			return { ...replayEvidence, id, status: updated.status, method: updated.method, url: replayEvidence.url ?? updated.url, responseStatus: updated.responseStatus ?? 0, note: updated.note, ...(updated.runOutput !== void 0 ? { runOutput: updated.runOutput } : {}), ...(typeof updated.responseBody === "string" && updated.responseBody !== "" ? { responseBody: updated.responseBody } : {}) };
		}
	}));
	ctx.tools.register(defineTool({
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			scenario: { type: "string", description: "普通 lesson 的触发场景；pattern 使用 recognize 四列替代。" },
			verificationPlaybook: { type: "string", description: "普通 lesson 的验证套路；pattern 使用 try 四列替代。" },
			acceptanceCriteria: { type: "string", description: "普通 lesson 的收录标准；pattern 使用 success 四列替代。" },
			...
			featureKeys: { type: "string", description: "pattern 目标特征：逗号分隔，如 api,auth,credential；不填则从四列和类型推导。" },
			kind: { type: "string", enum: ["lesson", "pattern"], description: "Phase 8：pattern=四列打法形态；普通经验省略或填 lesson。" },
			recognize: { type: "string", description: "pattern 四列①认什么：只写形态信号，不写具体域名/path/字段名。" },
			try: { type: "string", description: "pattern 四列②打哪：两句内说明不登录/需会话与试法。" },
			success: { type: "string", description: "pattern 四列③出什么算成：必须写相对基线的差分结果。" },
			falsePoint: { type: "string", description: "pattern 四列④假点：必填形态级证伪条件。" }
		},
		output: {
		...
		execute: async (args, exec) => {
			const kind = args.kind === "pattern" ? "pattern" : "lesson";
			if (kind === "pattern") {
				for (const key of PATTERN_COLUMNS) if (typeof args[key] !== "string" || args[key].trim() === "") throw new Error(`src_record_lesson pattern 缺少四列字段：${key}`);
				const shapeError = patternShapeError(args.recognize);
				if (shapeError !== "") throw new Error(`src_record_lesson pattern 拒绝：${shapeError}`);
				if (String(args.try).split(/[。.!！？?]/).filter((x) => x.trim() !== "").length > 2) throw new Error("src_record_lesson pattern 的「打哪」最多两句");
				if (!/(基线|对照|差分|多出|不同)/.test(String(args.success))) throw new Error("src_record_lesson pattern 的「出什么算成」必须写基线/对照差分");
			}
			const sessionId = sessionIdOf(exec);
			...
			if (kind === "pattern") {
				for (const row of await listAllLessons()) {
					try {
						const prior = patternOf(await fsPromises.readFile(nodePath.join(row.dir, `${row.file}.md`), "utf8"));
						if (prior !== null && prior.recognize.trim().toLowerCase() === String(args.recognize).trim().toLowerCase() && existing === "") throw new Error(`src_record_lesson pattern 拒绝新建：同型 pattern 已存在 ${row.file}，请读取后补充差分，不要重复建行`);
					} catch (error) { if (error instanceof Error && error.message.startsWith("src_record_lesson pattern 拒绝")) throw error; }
				}
			}
			const featureDecl = kind === "pattern" ? String(args.featureKeys ?? "").split(/[,，、;；\s]+/).map((s) => s.trim()).filter(Boolean).slice(0, 8) : [];
			const meta = JSON.stringify({ sessionId, vulnType: args.vulnType, kind, ...(featureDecl.length > 0 ? { featureKeys: featureDecl } : {}), sourceFindingTitle: typeof args.sourceFindingTitle === "string" ? args.sourceFindingTitle : "", createdAt: Date.now(), ...(Object.keys(triggerDecl).length > 0 ? { triggers: triggerDecl } : {}) });
			const body = [
				`# ${args.vulnType}`,
				...(kind === "pattern"
					? ["", `## 认什么`, args.recognize, "", `## 打哪`, args.try, "", `## 出什么算成`, args.success, "", `## 假点`, args.falsePoint, ""]
					: ["", `## 触发场景`, args.scenario, "", `## 验证套路`, args.verificationPlaybook, "", `## 收录标准`, args.acceptanceCriteria, ""]),
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			/* [local.94] Skill 提醒去重：读过的经验不再提醒。 */
			try { skillReminders.markRead(sessionIdOf(exec), String(args.id)); } catch {}
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			const sessionId = sessionIdOf(exec);
			/* [local.83 修] off 开关软降级：不写库、返回引导，行为与 off 不注册等价。 */
			if (srcSurveyFlag() === "off") return { action: args.action, degraded: true, note: "测绘种子功能已关闭（DSH_SRC_SURVEY=off）；如需启用设 DSH_SRC_SURVEY=shadow/on 后重启 dsh", guidance: "", equity: "" };
			...
			if (action === "ct") {
				...
				/* 两腿串行：agniops 子域清单（聚合库超集源，ct.name 数据是其子集故取之）+ certspotter 跨域 SAN 扩展（唯一能爆集团新根域）。均无需 key，国内直连。 */
				const legs = [];
				const rootsPending = [];
				let added = 0;
				for (const [provider, search] of [["agniops", agniopsSearch], ["certspotter", certspotterSearch]]) {

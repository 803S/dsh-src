	...
	*/
	...
	async materializeUnit(descriptor) {
		...
		if (row === void 0) db.prepare("INSERT INTO units (name, version) VALUES (?, ?)").run(descriptor.name, descriptor.version);
			...
			// 已知历史版本的兼容迁移（v6→v7、v7→v8、v8→v9、v9→v10、v10→v11、v11→v12）：只新增表（v7: observations/user_todos；
			// v8: infra；v9: test_accounts；v10: domain_notes；v12: pending_approvals）与记录级 optional/default 字段（v11: findings.status enum 加 active/rejected；v12: pending_approvals.method RUN/ASSET），旧行由 zod default 在读取时自动补齐，
			// 无需改写行数据。新表由下方 CREATE TABLE IF NOT EXISTS 统一确保。升级版本戳即完成迁移。
			db.prepare("UPDATE units SET version = ? WHERE name = ?").run(descriptor.version, descriptor.name);
			console.error(`[dsh-src] storage unit '${descriptor.name}' migrated v${row.version} -> v${descriptor.version}`);
		} else if (row.version > descriptor.version) throw new StorageError("version-mismatch", `kv unit '${descriptor.name}' is stamped version ${row.version} on the medium, newer than descriptor version ${descriptor.version}; downgrade not supported`);
...
function apply(ctx, config) {
	...
	ctx.effect(() => {
		const dispose = ctx.storage.backend.register("src-sqlite", backend);
		return async () => {
			dispose();
			await backend.close();
		};
	}, "storage-sqlite.registerBackend");
	ctx.provide(storageBackendServiceKey("src-sqlite"), backend);
}
//#endregion

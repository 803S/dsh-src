	...
	*/
	...
	async materializeUnit(descriptor) {
		...
		if (row === void 0) db.prepare("INSERT INTO units (name, version) VALUES (?, ?)").run(descriptor.name, descriptor.version);
			...
			// 已知历史版本的兼容迁移（v6→v7、v7→v8、v8→v9、v9→v10）：只新增表（v7: observations/user_todos；
			// v8: infra；v9: test_accounts；v10: domain_notes）与记录级可选/default 字段，旧行由 zod default 在读取时自动补齐，

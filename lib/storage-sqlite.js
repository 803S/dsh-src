	...
	*/
	...
	async materializeUnit(descriptor) {
		...
		if (row === void 0) db.prepare("INSERT INTO units (name, version) VALUES (?, ?)").run(descriptor.name, descriptor.version);
			...
			// 已知历史版本的兼容迁移（v6→v7）：v7 只新增 observations/user_todos 表与
			// 记录级可选/default 字段（finding entryPoint/discoveryPath/rawRequest/rawResponse、
			// checkpoint decision），旧行由 zod default 在读取时自动补齐，无需改写行数据。
			// 新表由下方 CREATE TABLE IF NOT EXISTS 统一确保。升级版本戳即完成迁移。

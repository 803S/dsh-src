...
function reportOf(src, t) {
	...
	const findingSections = findings.flatMap((finding) => {
		...
		const urlMatch = ((finding.pocEvidence ?? []).join("\n") + "\n" + (finding.steps ?? []).join("\n")).match(/https?:\/\/[^\s'"\\)]+/);
		const fullUrl = urlMatch === null ? asset === void 0 ? src.goal?.target ?? "" : asset.value : urlMatch[0];
		const rawPacket = [finding.rawRequest !== "" ? `=== Request ===\n${finding.rawRequest}` : "", finding.rawResponse !== "" ? `=== Response ===\n${finding.rawResponse}` : ""].filter((part) => part !== "").join("\n\n");
		return [

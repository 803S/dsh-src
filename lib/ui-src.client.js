	...
	factory: (require) => {
		...
		*/
		...
		function TodoListView({ src, t }) {
			const todos = Array.isArray(src.userTodos) ? src.userTodos : [];
		...
		function TimelineView({ src, t }) {
			...
			const factEvents = (src.facts ?? []).map((f) => ({ kind: "fact", id: f.id, parent: f.intentId, label: `[${f.kind}] ${f.detail}`, at: 0 }));
			const findingEvents = (src.findings ?? []).map((f) => ({ kind: "finding", id: f.id, parent: f.intentId, label: `[${f.severity}] ${f.title}`, at: 0 }));
			const intentEvents = (src.intents ?? []).map((i) => ({ kind: "intent", id: i.id, parent: null, label: `${i.status === "completed" ? "✓" : i.status === "blocked" ? "✗" : i.status === "failed" ? "!" : "·"} ${i.title}`, at: 0 }));
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("nav", {
						...
						children: TABS.map((tabKey) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							...
							children: [
								...
								tabKey === "timeline" ? ` (${(src.checkpoints?.length ?? 0) + (src.facts?.length ?? 0) + (src.findings?.length ?? 0)})` : ""
							]
						}, tabKey))
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		const zh = {
			"counts": "意图 {intents} · 事实 {facts} · 漏洞 {findings} · 资产 {assets} · 检查点 {checkpoints}",
		...
		const en = {
			"counts": "Intents {intents} · Facts {facts} · Findings {findings} · Assets {assets} · Checkpoints {checkpoints}",

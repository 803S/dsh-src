window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** [local.10] Infrastructure setting keys shown in the 基础设施 tab, with zh labels/placeholders. */
		const INFRA_FIELDS = [
			["proxyUrl", "HTTP 代理", "http://127.0.0.1:7890（留空=直连）"],
			["burpMcpPort", "Burp MCP 端口", "9876（仅记录，接线在 profile patch）"],
			["burpProxyJarPath", "mcp-proxy.jar 路径", "/path/to/mcp-proxy.jar（仅记录）"],
			["testAccount", "越权对照账号", "user:pass 或用户名；登录不了时 agent 会转用户待办"],
			["testPhone", "测试手机号（可多个）", "13800138000,13900139000（逗号分隔，仅限授权测试）"],
			["httpTimeoutMs", "HTTP 超时(ms)", "8000"]
		];
		...
		function InfraView({ src, t, runCommand }) {
			...
			const burpSnippet = [
				...
				"      config:",
				"        serverName: burp",
				`        command: java -jar <mcp-proxy.jar> --sse-url http://localhost:${resolved.burpMcpPort || "9876"}/  # 实测：扩展 SSE 在根路径且仅 HTTP`,
				"        failOnStartupError: false"
			].join("\n");
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { className: SrcView_module_css_default.list, children: [
				feedback !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { style: { color: busyKey !== null ? "#e80" : "#3c9", fontSize: 12, padding: "2px 4px" }, children: feedback }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { style: { fontSize: 12, color: "#888", margin: "2px 0 8px" }, children: "这些设置供本会话 agent 出站请求与测试使用；留空表示使用默认值（直连、端口 9876、超时 8000ms）。短信/越权类测试前，agent 必须先用 src_get_infra 读取这里的配置。" }),
				INFRA_FIELDS.map(([key, label, placeholder]) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }, children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", { style: { width: 170, flexShrink: 0, fontSize: 13 }, children: label }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", { value: draft[key] ?? resolved[key] ?? "", placeholder, onChange: (event) => setDraft((prev) => ({ ...prev, [key]: event.target.value })), onKeyDown: (event) => { if (event.key === "Enter") void save(key, draft[key] ?? ""); }, style: { flex: 1, minWidth: 160, fontSize: 12, padding: "5px 9px", borderRadius: 6, border: "1px solid rgba(127,127,127,.45)", background: "transparent", color: "inherit", fontFamily: key === "burpProxyJarPath" ? "ui-monospace, monospace" : "inherit" } }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", { type: "button", disabled: busyKey !== null, onClick: () => void save(key, draft[key] ?? ""), style: buttonStyle("#3c9"), children: busyKey === key ? "保存中…" : "保存" })
				] }, key)),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: 6, alignItems: "center", margin: "10px 0 0" }, children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", { type: "button", disabled: copyingInfra || busyKey !== null, onClick: async () => {
						setCopyingInfra(true);
						setFeedback(null);
						try {
							const result = await runCommand("/src-infra-copy");
							setFeedback(result.text ?? (result.kind === "success" ? "已沿用。" : "命令返回错误。"));
						} catch (error) {
							setFeedback(`发送失败：${error?.message ?? String(error)}`);
						} finally {
							setCopyingInfra(false);
						}
					}, style: buttonStyle("#3c9"), children: copyingInfra ? "沿用中…" : "沿用上次会话的基础设施" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: { fontSize: 11, color: "#888" }, children: "把最近配置过的其他会话的设置复制到本会话并覆盖当前值；新会话免重填" })
				] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: 6, alignItems: "center", margin: "10px 0 0" }, children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", { type: "button", disabled: proxyTesting || busyKey !== null, onClick: async () => {
						setProxyTesting(true);
						setFeedback(null);
						try {
							const result = await runCommand("/src-proxy-test");
							setFeedback(result.text ?? (result.kind === "success" ? "测活完成。" : "测活返回错误。"));
						} catch (error) {
							setFeedback(`发送失败：${error?.message ?? String(error)}`);
						} finally {
							setProxyTesting(false);
						}
					}, style: buttonStyle("#3c9"), children: proxyTesting ? "测活中…" : "测试代理连通" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: { fontSize: 11, color: "#888" }, children: "服务端直接经代理请求探针地址，结果只显示在这里，不打扰 agent" })
				] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { style: { marginTop: 10, fontSize: 12, padding: "8px 10px", borderRadius: 8, border: "1px solid rgba(127,127,127,.3)", background: "var(--dsw-alias-bg-layer-2, rgba(127,127,127,.08))" }, children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { style: { fontWeight: 600, marginBottom: 4 }, children: "Burp MCP 接线说明" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("ol", { style: { margin: "0 0 6px", paddingLeft: 18, lineHeight: 1.7 }, children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: "Burp Pro 装 \"MCP Server\" 扩展并点 Start（默认监听 9876 端口）。" }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", { children: [
							"确认 profile 的 cordis.patch.yml 里 mcp-burp 段的 jar 路径存在（本机默认 ",
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { style: { fontFamily: "ui-monospace, monospace" }, children: "~/.dsh/profiles/web/cordis.patch.yml" }),
							"，改后需重启 dsh web）。"
						] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: "点下方按钮：先探测 Burp 端口是否监听（不通会直接给出原因），通了再让 agent 做 MCP 层验证。" })
					] }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", { type: "button", disabled: burpTesting || busyKey !== null, onClick: async () => {
						setBurpTesting(true);
						setFeedback(null);
						try {
							const result = await runCommand("/src-burp-test");
							setFeedback(result.text ?? (result.kind === "success" ? "测试请求已发出。" : "测试返回错误。"));
						} catch (error) {
							setFeedback(`发送失败：${error?.message ?? String(error)}`);
						} finally {
							setBurpTesting(false);
						}
					}, style: buttonStyle("#3c9"), children: burpTesting ? "发送中…" : "测试 Burp MCP 连接" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { style: { color: "#888", marginTop: 4 }, children: "上方「Burp MCP 端口 / jar 路径」两项仅作记录供 agent 参考，不会改动任何配置文件；实际接线只看 cordis.patch.yml。" })
				] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", { style: { marginTop: 10, fontSize: 12 }, children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", { style: { cursor: "pointer", color: "#888" }, children: "cordis.patch.yml 片段示例（mcp-burp 段）" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", { style: preStyle(220), children: burpSnippet })
				] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { style: { fontSize: 12, color: "#888", marginTop: 10 }, children: [
					"进程级代理：若希望 dsh 主进程所有流量都走系统代理，启动前设置环境变量 ",
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { style: { fontFamily: "ui-monospace, monospace" }, children: "HTTPS_PROXY=http://127.0.0.1:7890 NODE_OPTIONS=--use-env-proxy" }),
					"。插件内的 HTTP 代理仅作用于 SRC 出站请求。"
				] })
			] });
		}

...
function reportOf(src, t) {
	...
	const findingSections = src.nodes.filter((node) => node.kind === "finding").filter((finding) => (finding.status ?? "active") === "active").flatMap((finding) => {
		const asset = finding.affectedAssetId === void 0 ? void 0 : src.assets.find((candidate) => candidate.id === finding.affectedAssetId);
		const urls = (() => {
...
function scopeLimitationLines(src, t) {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
				...
				children: [
					...
					(() => {
						const rows = (Array.isArray(src.coverage) ? src.coverage : []).filter((r) => typeof r.endpointsTotal === "number" && r.endpointsTotal > 0);
						if (rows.length === 0) return null;
						const tested = rows.reduce((s, r) => s + (r.endpointsTested ?? 0), 0);
						const total = rows.reduce((s, r) => s + (r.endpointsTotal ?? 0), 0);
						const skipped = rows.reduce((s, r) => s + (r.endpointsSkipped ?? []).length, 0);

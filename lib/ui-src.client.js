...
function blindSpotLines(src, t) {
	const rows = (src.coverage ?? []).filter((row) => {
		...
		return row.phase === "blind-spot";
	});
	if (rows.length === 0) return [t("report.blindSpotsEmpty")];
	const statusLabel = (status) => status === "completed" ? t("report.blindSpotCovered") : status === "blocked" ? t("report.blindSpotUncovered") : status === "not-applicable" ? t("report.blindSpotNa") : String(status);
	return rows.map((row) => {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					...
					title: "把最近配置过的其他会话的设置复制到本会话并覆盖当前值；新会话免重填",

	...
	factory: (require) => {
		...
		*/
		...
		function TimelineView({ src, t }) {
			...
			const ckEvents = checkpoints.map((c) => ({ kind: "checkpoint", id: c.id, parent: c.intentId, label: `${c.stage} · +${c.facts}f/+${c.assets}a/+${c.findings}v · ${c.summary || ""}`, at: c.createdAt }));
			const events = [...intentEvents, ...factEvents, ...findingEvents, ...ckEvents].sort((a, b) => (a.at - b.at) || a.id.localeCompare(b.id));
			...
			const color = (k) => k === "finding" ? "#d33" : k === "fact" ? "#39c" : k === "checkpoint" ? "#999" : "#3c9";

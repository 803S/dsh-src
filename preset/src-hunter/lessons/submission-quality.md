...
- **报告渲染**：报告按美团 SRC 骨架模板生成（4 字段 + 漏洞风险详情 4 小节），内容动态填充。attackChain 作为发现→利用→危害的闭合叙事直接融入第 1 节主叙述，不单独列块；发现方式/入口/应用下载等「根据实际情况」字段非空才显示、不占位。

## 报告模板（美团 SRC 骨架，内容动态填充）
报告按美团 SRC 官方模板生成，可直接贴给厂商无需手改格式。4 个字段（漏洞名称/漏洞类型/漏洞URL/漏洞级别）+ 漏洞风险详情四小节（描述&发现方式、利用及危害 / 详细复现证明 / 测试源信息 / 修复方案）。**内容动态填充，空字段不显示、不占位、不凑数**。
- **vulnType**（漏洞类型）：类别名，如「登录认证漏洞」「越权漏洞」「信息泄露」「短信轰炸」「文件上传漏洞」「SSRF」「XSS」「逻辑漏洞」。
- **attackChain**（攻击链）：多跳/组合利用必填的闭合叙事；它本身就是第 1 节主叙述，报告不单列块。单步漏洞可不填，报告由结构化字段填充。
...
- **「根据实际情况」字段**（发现方式/入口/应用下载/影响范围）：不是固定行，填了才显示、不填不占位。App 下载仅 app/小程序资产才带。

...
type TodoRow = {
  ...
  readonly createdAt?: number
		...
		*/
		...
		function ApprovalListView({ src, t, runCommand }: { readonly src: SrcProjection; readonly t: PropsLocale['t']; readonly runCommand?: ((cmd: string) => Promise<{ kind: string; text: string }>) | undefined }) {
			...
			if (approvals.length === 0) return<div className={css.empty}>暂无待审请求（高危删改/越权请求、资产归属确认会挂在这里等你决定）</div>;
			...
			return<div className={css.list}>{feedback !== null &&<div style={{ color: busyId !== null ? "var(--dsw-alias-state-warn-primary, #e80)" : "var(--dsw-alias-state-error-primary, #c33)", fontSize: 12, padding: "2px 4px" }}>{feedback}</div>}{approvals.map((ap) =><div className={css.card} style={{ borderLeft: `3px solid ${ap.status === "approved" ? "var(--dsw-alias-state-success-primary, #3c9)" : ap.status === "rejected" ? "var(--dsw-alias-label-tertiary, #999)" : "var(--dsw-alias-state-error-primary, #c33)"}`, marginBottom: 4, padding: "6px 10px", fontSize: 13 }}><div>{ap.method === "ASSET" ? "🏷️" : "⚠️"} <strong>{ap.method === "ASSET" ? `资产归属确认：${ap.url}` : `${ap.method} ${ap.url}`}</strong> <span style={{ color: "var(--dsw-alias-state-error-primary, #c33)" }}>{`[${ap.category}]`}</span><span style={{ color: "var(--dsw-alias-label-caption, #aaa)", marginLeft: 6, fontSize: 11 }}>{ap.id}</span></div>{ap.status !== "pending" &&<div style={{ color: ap.status === "approved" ? "var(--dsw-alias-state-success-primary, #3c9)" : "var(--dsw-alias-label-tertiary, #999)", fontSize: 12 }}>{ap.status === "approved" ? (ap.method === "ASSET" ? "已确认归属（整域授权）" : `已批准${ap.responseStatus > 0 ? ` → 响应 ${ap.responseStatus}` : ""}`) : (ap.method === "ASSET" ? "已否决（整域排除）" : "已拒绝")}{ap.note !== "" ? `· 备注：${ap.note}` : ""}</div>}{ap.justification !== "" &&<div style={{ color: "var(--dsw-alias-label-tertiary, #888)" }}>{ap.justification}</div>}{ap.reason !== "" &&<div style={{ color: "var(--dsw-alias-label-tertiary, #888)", fontSize: 12 }}>{ap.method === "ASSET" ? "判定依据：" : "分类理由："}{ap.reason}</div>}{ap.method !== "ASSET" &&<details style={{ marginTop: 2 }}><summary style={{ cursor: "pointer", color: "var(--dsw-alias-label-tertiary, #888)", fontSize: 11 }}>请求报文</summary><pre style={preStyle(240)}>{ap.method} {ap.url}{ap.headers !== "" ? `\n${ap.headers}` : ""}{ap.body !== "" ? `\n\n${ap.body}` : ""}</pre></details>}{ap.status === "pending" && runCommand !== void 0 &&<div style={{ marginTop: 4 }}>{noteFor?.id === ap.id ?<><div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 8, border: "1px solid rgba(127,127,127,.3)", background: "var(--dsw-alias-bg-layer-2, rgba(127,127,127,.08))" }}><div style={{ fontSize: 11, color: "var(--dsw-alias-label-tertiary, #888)", marginBottom: 5 }}>{(noteFor as { action: string }).action === "allow" ? (ap.method === "ASSET" ? "确认归属——补充备注（可选）" : "批准——补充备注（可选，会转达给 AI）") : (ap.method === "ASSET" ? "否决——说明原因（可选）" : "拒绝——说明原因（可选，会转达给 AI）")}</div><textarea autoFocus={true} rows={3} value={noteText} placeholder={(noteFor as { action: string }).action === "allow" ? (ap.method === "ASSET" ? "如：确实是目标公司的注册域\n（Enter 发送，Shift+Enter 换行）" : "如：可信测试账号、是自己的资源\n（Enter 发送，Shift+Enter 换行）") : "如：不批准，可能误伤真实用户\n（Enter 发送，Shift+Enter 换行）"} onChange={(event) => setNoteText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendApproval(ap.id, (noteFor as { action: string }).action, noteText); } if (event.key === "Escape") setNoteFor(null); }} style={{ display: "block", width: "100%", boxSizing: "border-box", fontSize: 13, padding: "6px 10px", borderRadius: 6, border: "1px solid rgba(127,127,127,.4)", background: "transparent", color: "inherit", resize: "vertical", minHeight: 60, fontFamily: "inherit", lineHeight: 1.5 }} /><div style={{ display: "flex", gap: 6, justifyContent: "flex-end", alignItems: "center", marginTop: 7 }}><button type="button" disabled={busyId !== null} onClick={() => void sendApproval(ap.id, (noteFor as { action: string }).action, noteText)} style={buttonStyle((noteFor as { action: string }).action === "allow" ? "var(--dsw-alias-state-success-primary, #3c9)" : "var(--dsw-alias-label-tertiary, #999)")}>{busyId !== null ? "发送中…" : "发送"}</button><button type="button" onClick={() => setNoteFor(null)} style={buttonStyle("var(--dsw-alias-label-tertiary, #888)")}>取消</button></div></div></> :<div style={{ display: "flex", gap: 6, alignItems: "center" }}><button type="button" disabled={busyId !== null} onClick={() => { setNoteText(""); setNoteFor({ id: ap.id, action: "allow" }); }} style={buttonStyle("var(--dsw-alias-state-success-primary, #3c9)")}>{ap.method === "ASSET" ? "✓ 确认归属" : "✓ 批准"}</button><button type="button" disabled={busyId !== null} onClick={() => { setNoteText(""); setNoteFor({ id: ap.id, action: "reject" }); }} style={buttonStyle("var(--dsw-alias-label-tertiary, #999)")}>{ap.method === "ASSET" ? "✗ 非本组织" : "✗ 拒绝"}</button>{busyId === ap.id &&<span style={{ color: "var(--dsw-alias-state-warn-primary, #e80)", fontSize: 12 }}>发送中…</span>}</div>}</div>}</div>)}</div>;
		}
  ...
  readonly runCommand?: ((cmd: string) => Promise<{ kind: string; text: string }>) | undefined
			...
			return<section className={css.root} data-testid="src-view"><header className={css.card}>{src.goal !== null && src.goal.objective !== "" && <p className={css.objective}>目的：{src.goal.objective}</p>}<div className={css.cardTitle} style={{ alignItems: "center" }}><h2 className={css.target} style={{ fontSize: 18 }}>{src.goal === null ? "" : src.goal.target}</h2>{src.goal !== null && src.goal.authorization !== "" && <span className={css.badge} title={src.goal.authorization}>授权：{src.goal.authorization.length > 24 ? `${src.goal.authorization.slice(0, 24)}…` : src.goal.authorization}</span>}</div>{(() => {
							...
							})()}{src.apiDiscovery && (src.apiDiscovery.total ?? 0) > 0 &&<p className={css.counts}>API 发现：{src.apiDiscovery.total ?? 0} · schema {src.apiDiscovery.schemas ?? 0} · GraphQL {src.apiDiscovery.graphql ?? 0} · hint {src.apiDiscovery.hints ?? 0} · 未推进 {src.apiDiscovery.untouched ?? 0}</p>}{(() => { /* [local.82] 接口对账数字归运营面：报告已瘦身，对账随头部统计区呈现 */ const rows = (Array.isArray(src.coverage) ? src.coverage : []).filter((r) => typeof r.endpointsTotal === "number" && r.endpointsTotal > 0); if (rows.length === 0) return null; const tested = rows.reduce((s, r) => s + (r.endpointsTested ?? 0), 0); const total = rows.reduce((s, r) => s + (r.endpointsTotal ?? 0), 0); const skipped = rows.reduce((s, r) => s + (r.endpointsSkipped ?? []).length, 0); return <p className={css.counts}>接口对账：实测 {tested} / 清单 {total}{skipped > 0 ? `（跳过 ${skipped}）` : ""}</p>; })()}</header><nav className={css.tabs} data-testid="src-tabs">{TABS.map((tabKey) =><button type="button" className={css.tab} aria-pressed={tab === tabKey} data-testid={`src-tab-${tabKey}`} onClick={() => {
								setTab(tabKey);
							}}>{t(TAB_LABELS[tabKey])}{(() => {

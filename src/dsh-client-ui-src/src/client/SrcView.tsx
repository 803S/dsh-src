  ...
  readonly createdAt?: number
		...
		*/
		...
		function TodoListView({ src, t, runCommand }: { readonly src: SrcProjection; readonly t: PropsLocale['t']; readonly runCommand?: ((cmd: string) => Promise<{ kind: string; text: string }>) | undefined }) {
			...
			return<div className={css.list}>{feedback !== null &&<div style={{ color: busyId !== null ? "#e80" : "#c33", fontSize: 12, padding: "2px 4px" }}>{feedback}</div>}{todos.map((todo) =><div className={css.card} style={{ borderLeft: `3px solid ${todo.status === "done" ? "#3c9" : todo.status === "abandoned" ? "#999" : "#e80"}`, marginBottom: 4, padding: "6px 10px", fontSize: 13 }}><div>{todo.status === "done" ? "\u2705" : todo.status === "abandoned" ? "\u274C" : "\u26A0\uFE0F"} <strong>{todo.title}</strong> <span style={{ color: "#888" }}>{`[${todo.kind}]`}</span><span style={{ color: "#aaa", marginLeft: 6, fontSize: 11 }}>{todo.id}</span></div>{todo.detail !== "" &&<div>{todo.detail}</div>}{todo.note !== "" &&<div style={{ color: "#3c9" }}>用户备注：{todo.note}</div>}{todo.status === "pending" && runCommand !== void 0 &&<div style={{ marginTop: 4 }}>{noteFor?.id === todo.id ?<><div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 8, border: "1px solid rgba(127,127,127,.3)", background: "var(--dsw-alias-bg-layer-2, rgba(127,127,127,.08))" }}><div style={{ fontSize: 11, color: "#888", marginBottom: 5 }}>{(noteFor as { status: string }).status === "done" ? "\u2713 标记为已完成 \u2014\u2014 补充备注（可选，会转达给 AI）" : "\u2717 放弃此项 \u2014\u2014 说明原因（可选，会转达给 AI）"}</div><textarea autoFocus={true} rows={3} value={noteText} placeholder={(noteFor as { status: string }).status === "done" ? "如：已用 Burp 抓好登录包\n（Enter 发送，Shift+Enter 换行）" : "如：暂不测短信，跳过这项\n（Enter 发送，Shift+Enter 换行）"} onChange={(event) => setNoteText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendTodoFeedback(todo.id, (noteFor as { status: string }).status, noteText); } if (event.key === "Escape") setNoteFor(null); }} style={{ display: "block", width: "100%", boxSizing: "border-box", fontSize: 13, padding: "6px 10px", borderRadius: 6, border: "1px solid rgba(127,127,127,.4)", background: "transparent", color: "inherit", resize: "vertical", minHeight: 60, fontFamily: "inherit", lineHeight: 1.5 }} /><div style={{ display: "flex", gap: 6, justifyContent: "flex-end", alignItems: "center", marginTop: 7 }}><button type="button" disabled={busyId !== null} onClick={() => void sendTodoFeedback(todo.id, (noteFor as { status: string }).status, noteText)} style={buttonStyle((noteFor as { status: string }).status === "done" ? "#3c9" : "#999")}>{busyId !== null ? "发送中…" : "发送"}</button><button type="button" onClick={() => setNoteFor(null)} style={buttonStyle("#888")}>取消</button></div></div></> :<div style={{ display: "flex", gap: 6, alignItems: "center" }}><button type="button" disabled={busyId !== null} onClick={() => { setNoteText(""); setNoteFor({ id: todo.id, status: "done" }); }} style={buttonStyle("#3c9")}>✓ 我已完成</button><button type="button" disabled={busyId !== null} onClick={() => { setNoteText(""); setNoteFor({ id: todo.id, status: "abandoned" }); }} style={buttonStyle("#999")}>✗ 放弃此项</button>{busyId === todo.id &&<span style={{ color: "#e80", fontSize: 12 }}>发送中…</span>}</div>}</div>}</div>)}</div>;
		}
		...
		function timelineDetailRow(label: string, value: unknown): ReactNode {
			...
			return<div style={{ margin: "2px 0", wordBreak: "break-all" }}><span style={{ color: "#888", fontSize: 11, marginRight: 6 }}>{label}</span>{String(value)}</div>;
		}
		...
		function TimelineView({ src, t }: { readonly src: SrcProjection; readonly t: PropsLocale['t'] }) {
			...
			const intentStatusMark = (status: string) => status === "completed" ? "✓" : status === "blocked" ? "✗" : status === "failed" ? "!" : "·";
			...
			const intentEvents = (src.nodes ?? []).filter((node: any) => node.kind === "intent").map((i: any) => ({ kind: "intent", id: i.id, parent: null, title: i.title, detail: i.detail, status: i.status, at: i.createdAt ?? 0, short: `[意图] ${intentStatusMark(i.status)} ${i.title}` }));
			...
			const color = (k: string) => k === "finding" ? "#d33" : k === "fact" ? "#39c" : k === "checkpoint" ? "#999" : k === "observation" ? "#e80" : "#3c9";
			...
			return<div className={css.list} style={{ display: "flex", flexDirection: "column", gap: 6 }}><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{chips.map(([key, label]) =><button type="button" onClick={() => setFilter(key)} style={{ cursor: "pointer", fontSize: 12, padding: "1px 10px", borderRadius: 999, border: `1px solid ${color(key === "all" ? "#888" : key)}`, background: filter === key ? "rgba(127,127,127,.18)" : "transparent", color: "inherit" }}>{label}</button>)}</div><div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "stretch" }}><div style={{ flex: "1 1 240px", minWidth: 220, maxHeight: 430, overflowY: "auto", borderRight: "1px solid rgba(127,127,127,.25)", paddingRight: 4 }} data-testid="src-timeline-list">{filtered.map((e) => {
						...
						return<div onClick={() => setSelectedKey(key)} style={{ cursor: "pointer", display: "flex", gap: 6, alignItems: "baseline", padding: "3px 6px", borderRadius: 6, marginBottom: 2, fontSize: 12, background: selectedKey === key || selected === e ? "rgba(127,127,127,.16)" : "transparent" }} data-testid="src-timeline-item"><span style={{ color: color(e.kind), flexShrink: 0 }}>●</span><span style={{ color: "#888", fontVariantNumeric: "tabular-nums", flexShrink: 0 }}>{fmtTime(e.at)}</span><span style={{ wordBreak: "break-all" }}>{e.short}</span></div>;
